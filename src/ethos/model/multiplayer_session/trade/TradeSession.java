...
/*				PlayerLogging.write(LogType.TRADE_LOG, player,
	...
	public boolean itemAddable(Player player, GameItem item) {
		if (item.getId() == 12650 || item.getId() == 12649 || item.getId() == 12651 || item.getId() == 12652 || item.getId() == 12644 || item.getId() == 12645 || item.getId() == 12643 || item.getId() == 11995 || item.getId() == 15568 || item.getId() == 12653 || item.getId() == 12655 || item.getId() == 13178 || item.getId() == 12646 || item.getId() == 13179 || item.getId() == 13177 || item.getId() == 12921 || item.getId() == 13181 || item.getId() == 12816 || item.getId() == 12647 && !player.getRights().isOrInherits(Right.BRNZ)) {
			player.sendMessage("You cannot trade this item, it is deemed as untradable.");
			return false;
		}
		if (item.getId() == 12006 && !player.getRights().isOrInherits(Right.BRNZ)) {
			player.sendMessage("You cannot trade this item, it is deemed as untradable.");
			return false;
		}
		if (!player.getItems().isTradable(item.getId()) && !player.getRights().isOrInherits(Right.BRNZ)) {
			player.sendMessage("You cannot trade this item, it is deemed as untradable.");
			return false;
		}
