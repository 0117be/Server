...
public class Trade extends Multiplayer {
	...
	public boolean requestable(Player requested) {
		...
		if (player.getRights().isOrInherits(Right.BRNZ) && !requested.getRights().isOrInherits(Right.BRNZ) && p.clan.activeMembers.size() >= 1 && player.clan.activeMembers.size() >= 1) {
			player.sendMessage("You can only trade other Bronze members in your clan chat.");
			return false;
		}
