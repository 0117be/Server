...
public class ShopAssistant {
	...
	public void buyFromShopPrice(int removeId, int removeSlot) {
		...
		if (c.myShopId == 83) {
			c.sendMessage("@blu@You can only sell items to this shop");
			return;
		}
		if (c.myShopId == 85) {
			c.sendMessage("@blu@You can only sell items to this shop");
		...
		if (c.myShopId == 136) {
			c.sendMessage(ItemAssistant.getItemName(removeId) + ": currently costs " + getSpecialItemValue(removeId) + " Boss Points.");
			return;
		}
		if (c.myShopId == 13) {
		...
		if (c.myShopId == 10) {
			c.sendMessage(ItemAssistant.getItemName(removeId) + ": currently costs [ @pur@" + getSpecialItemValue(removeId) + " @bla@] Slayer Points.");
			return;
		}
		if (c.myShopId == 120) {
		...
		if (c.myShopId == 128) {
			c.sendMessage(ItemAssistant.getItemName(removeId) + ": currently costs [ @pur@" + getSpecialItemValue(removeId) + " @bla@] Daily Points.");
			return;
		}
		if (c.myShopId == 119) {
		...
		if (c.myShopId == 9 || c.myShopId == 137) {
			c.sendMessage(ItemAssistant.getItemName(removeId) + ": currently costs [ @blu@" + getSpecialItemValue(removeId) + " @bla@ ] Donator Coins.");
			return;
		}
		if (c.myShopId == 18) {
		...
		switch (c.myShopId) {
			...
			case 44: //Slayer Shop
				switch (id) {
					...
					case 19529:
						return 400;
					case 12783://Ring of Wealth Scroll
						return 425;
				}
				break;
			...
			case 136: 
				...
				case 30020://PERK
					...
					case 11730://overloads
						return 1;
					
					case 12695://Super cmb pot
						return 1;
			...
			case 77: //Vote Shop
				switch (id) {
					...
					case 13233:
						return 45;
					/**
					 * Bonus XP books Vote store
					 */
					case 13513://30 min book
						return 7;
					case 14945://15 min XP book
						return 3;
					case 14947://60 min XP book
						return 14;
					/**
					 * Bonus drop books vote store
					 */
					case 13279://15 min drop book
						return 20;
					case 14955://30 min drop book
						return 35;
					case 14957://60 min drop book
						return 60;
					...
					case 4039://Zamorak banner
					case 6583://Ring of Stone
					case 7927://Easter Ring
					case 20005://Ring of Nature
					case 20017://Ring of Coins
					case 21817://Bracelet of Etherum
					...
					case 4565://Basket of Eggs
						return 150;
					case 964://Postie Pete Pet
						return 275;
					case 13346://Ultra Mystery Box
						return 150;
				}
				break;
			case 128: //Daily Point Shop
				switch (id) {
					case 20708://pyromancer
					case 20704:
					case 20706:
					case 20710:
					case 10941://lumberjack
					case 10939:
					case 10940:
					case 10933:
						return 12;

					case 12013://prospector
					case 12014:
					case 12015:
					case 12016:
					case 13258://angler
					...
					case 6199:
						return 50;
					case 21126://double xp ring
						return 185;
					/**
					 * Double Drop books inside Daily Point store
					 */
					case 13279://15 min drop book
						return 30;
					case 14955://30 min drop book
					...
					/**
					 * Bonus XP books inside Daily Point store
					 */
					case 14945://15 min XP book
						return 10;
					case 13513://30 min XP book
						return 18;
					case 14947://60 min XP book
						return 35;
				}
			...
			case 9: //Donator Point Shop
				switch (id) {
					...
					case 21295:
						return 30;
					//Bonus Xp books
					case 14945://15 minutes
						return 3;
					case 13513://30 minutes
						return 6;
					case 14947://1 hour
						return 11;
					case 14951://1 hour 30 min
						return 18;
					case 14949://3 hours
						return 30;
					//Double Drop Books
					case 13279://15 minutes
						return 6;
					case 14955://30 minutes
						return 12;
					case 14957://1 hour
						return 20;
					case 14953://1 hour 30 mins
						return 28;
					case 14959://3 hours
						return 40;
					//Raid Prayer Scrolls
	...
	public void sellToShopPrice(int removeId, int removeSlot) {
		...
		if (IsIn == false) {
			...
			if (c.myShopId == 83 || c.myShopId == 85 || c.myShopId == 152) {
				int i = 0;
			...
			if (c.myShopId == 12) {
				c.sendMessage(ItemAssistant.getItemName(removeId) + ": shop will buy for " + ((int) Math.ceil((getSpecialItemValue(removeId) * 0.20)) + " PKP Tickets."));
				...
				ShopValue *= 0.750;
				c.sendMessage(ItemAssistant.getItemName(removeId) + ": shop will buy for " + ShopValue + " tokkul");
			}  else if (c.myShopId == 83 || c.myShopId == 85 || c.myShopId == 152) {
				ShopValue *= 1.000;
	...
	public boolean sellItem(int itemID, int fromSlot, int amount) {
		...
		if (amount > 0 && itemID == (c.playerItems[fromSlot] - 1)) {
			...
			if (c.myShopId != 126 && c.myShopId != 44 && c.myShopId != 2) {
				TotPrice2 *= 0.667;
			...
			if (c.getItems().freeSlots() > 0 || c.getItems().playerHasItem(995)) {
				if ((Item.itemStackable[itemID] || Item.itemIsNote[itemID]) && c.getItems().playerHasItem(itemID, amount)) {
					...
					if (c.myShopId == 12) {
						c.getItems().addItem(2996, (int) Math.ceil(TotPrice3 *= 0.20));
					} else if (c.myShopId != 12 && c.myShopId != 29 && c.myShopId != 44 && c.myShopId != 18  && c.myShopId != 83 && c.myShopId != 85 && c.myShopId != 152 && c.myShopId != 116 && c.myShopId != 115  && c.myShopId != 136) {
						...
						Achievements.increase(c, AchievementType.GAINS, 1);
					} else if (c.myShopId == 151) {					
						Achievements.increase(c, AchievementType.PERK, 1);
					}  else if (c.myShopId == 116) {
	...
	public void handleOtherShop(int itemID, int amount) {
		...
		if (c.myShopId == 136) {
			...
			int itemValue = getSpecialItemValue(itemID) * amount;
			if (c.getBossPoints() < itemValue) {
				c.sendMessage("You do not have enough boss points to buy this from the shop.");
				return;
			}
			c.bossPoints =(c.getBossPoints() - itemValue);
			c.getItems().addItem(itemID, amount);
			c.getItems().resetItems(3823);
			c.refreshQuestTab(5);
			return;
		...
		if (c.myShopId == 128 && itemID == 3025 || itemID == 6686) {
			int itemValue = getSpecialItemValue(itemID) * amount;
			if (c.dailyPoints < itemValue) {
				c.sendMessage("You do not have enough pk points to buy this item.");
				return;
			}
			c.pkp -= itemValue;
		...
		} else if (c.myShopId == 128) {
			if (c.dailyPoints >= getSpecialItemValue(itemID) * amount) {
				if (c.getItems().freeSlots() > 0) {
					c.dailyPoints -= getSpecialItemValue(itemID) * amount;
					c.refreshQuestTab(1);
					c.refreshQuestTab(5);
					c.getItems().addItem(itemID, amount);
					c.getItems().resetItems(3823);
		...
		} else if (c.myShopId == 9 || c.myShopId == 137) {
			int itemValue = getSpecialItemValue(itemID) * amount;
			if (!c.getItems().playerHasItem(30230, itemValue)) {
				c.sendMessage("You do not have enough Donator Coins to buy this item.");
				return;
			} else {
				c.getItems().deleteItem(30230, itemValue);
				c.getItems().addItem(itemID, amount);
				c.getItems().resetItems(3823);
				return;
			}

		} else if (c.myShopId == 136) {
			int itemValue = getSpecialItemValue(itemID) * amount;
			if (!c.getItems().playerHasItem(3823, itemValue)) {
				c.sendMessage("You do not have enough Boss Points to buy this item.");
				return;
			} else {
				c.getItems().addItem(itemID, amount);
