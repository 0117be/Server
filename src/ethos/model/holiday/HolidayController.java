...
public class HolidayController {
	...
	public static final Christmas CHRISTMAS = new Christmas("Christmas", new GregorianCalendar(Server.getCalendar().getInstance().get(Calendar.YEAR), Calendar.NOVEMBER, 5, 0, 0),
			new GregorianCalendar(Server.getCalendar().getInstance().get(Calendar.YEAR), Calendar.NOVEMBER, 30, 0, 0), new ChristmasCycleEvent());
	...
	public void giveReward(Player player, Holiday holiday) {
		...
		if (Objects.nonNull(op) && op.isPresent()) {
			...
			h.receive(player);
			player.getItems().addItemUnderAnyCircumstance(10508, 1);
