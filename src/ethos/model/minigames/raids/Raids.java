...
public class Raids {
	...
	int[] rarerewards = { 21034, 21079, 20784, 21000, 21006, 21012, 21015, 21018, 21021, 21024, 20997, 21003 };
	...
	public void giveRareReward() {
		...
		PlayerHandler.executeGlobalMessage("<shad=7000>[COX]@gre@ @cr36@ " + player.playerName + " has received a rare item @gre@"+ ItemDefinition.forId(player.raidReward[0][0]).getName() + " from COX!");
		player.talkingNpc = 8324;
		player.getDH().sendNpcChat("You are extremely lucky! Here is a " + ItemDefinition.forId(player.raidReward[0][0]).getName());
		...
		switch(npcType) {
			...
			case 7554:
				for (String username : player.getRaids().raidLeader.clan.activeMembers) {
					...
					p.getRaids().updateRaidPoints();
					p.getPA().movePlayer(2400, 5542, 4);
				}
				return;		
	...
	public boolean handleObjectClick(Player player, int objectId) {
		switch(objectId) {
			...
			case 30028:	
				if (player.getItems().isWearingItem(9104) && Calendar.getInstance().get(Calendar.DAY_OF_WEEK) == Calendar.SATURDAY && player.getItems().getItemCount(30012, true) != 1) {
					player.getRaids().giveReward();				
					player.getItems().addItemUnderAnyCircumstance(player.raidReward[0][0], player.raidReward[0][1]);
					player.sendMessage("@red@You have been given 3 rewards for wearing the raids ring on a Saturday!");
					player.sendMessage("@red@You have been given 5 extra points for wearing the raids ring!");
					player.sendMessage("@red@You have completed "+player.raidCount+" raids.");
					player.getRaids().giveReward();	
					player.raidCount+=1;
					player.getItems().addItemUnderAnyCircumstance(player.raidReward[0][0], player.raidReward[0][1]);
					player.getPA().movePlayer(1255,3562,0);	
					player.getRaids().roomNames = new ArrayList<String>();
					player.getRaids().roomPaths= new ArrayList<Location>();
					player.getRaids().currentRoom = 0;
					player.getRaids().mobAmount=0;
					...
					player.getRaids().skilling = false;
					...
					PlayerHandler.executeGlobalMessage("<shad=7000>@red@[COX] Someone just completed the Chambers of Xeric!");
					player.raidPoints+=10;
					player.hasRaidOverloadBoost = false;
					  if (Misc.random(75) == 1) {
				            PlayerHandler.executeGlobalMessage("<shad=7000>@gre@[Cox] " + player.playerName
				                    + " has recieved a Cox Crate!");
				            player.getItems().addItemUnderAnyCircumstance(19903, 1);
				        }
					break;
				}
				...
				if (player.getItems().isWearingItem(9104) && Calendar.getInstance().get(Calendar.DAY_OF_WEEK) != Calendar.SATURDAY && player.getItems().getItemCount(30012, true) != 1) {
					player.getRaids().giveReward();				
					player.getItems().addItemUnderAnyCircumstance(player.raidReward[0][0], player.raidReward[0][1]);
					player.sendMessage("@red@You have been given an extra reward for wearing the raids ring!");
					player.sendMessage("@red@You have been given 3 extra points for wearing the raids ring!");
					player.sendMessage("@red@You have completed "+player.raidCount+" raids.");
					player.getRaids().giveReward();	
					player.raidCount+=1;
					player.getItems().addItemUnderAnyCircumstance(player.raidReward[0][0], player.raidReward[0][1]);
				 ...
				 if (player.getItems().getItemCount(30012, true) == 1 && player.getItems().isWearingItem(9104) && Calendar.getInstance().get(Calendar.DAY_OF_WEEK) != Calendar.SATURDAY) {	
						player.getRaids().giveReward();				
						player.getItems().addItemUnderAnyCircumstance(player.raidReward[0][0], player.raidReward[0][1]);
						player.sendMessage("@red@You have been given an extra reward for wearing the raids ring!");
						player.sendMessage("@red@You have been given 7 extra raid points from your perk with the raids ring!");
						player.sendMessage("@red@You have completed "+player.raidCount+" raids." );
						player.raidCount+=1;
						player.getPA().movePlayer(1255,3562,0);
						player.getRaids().roomNames = new ArrayList<String>();
				  ...
				  if (player.getItems().getItemCount(30012, true) == 1  && !player.getItems().isWearingItem(9104)) {	
					player.getRaids().giveReward();				
					player.getItems().addItemUnderAnyCircumstance(player.raidReward[0][0], player.raidReward[0][1]);
					player.sendMessage("@red@You have been given 5 extra raid points from your perk!");
					player.sendMessage("@red@You have completed "+player.raidCount+" raids." );
					player.raidCount+=1;
					player.getPA().movePlayer(1255,3562,0);
					...
					if (Calendar.getInstance().get(Calendar.DAY_OF_WEEK) == Calendar.SATURDAY) {
						...
						player.sendMessage("@red@You have been given 2 rewards today only!");
						PlayerHandler.executeGlobalMessage("<shad=7000>@red@[COX] Someone just completed the Chambers of Xeric!");
					//	DiscordMessager.sendAnnouncement("```[Raids] Someone just completed the Chambers of Xeric!```");
						player.sendMessage("@red@You have completed "+player.raidCount+" raids." );			
						player.getRaids().roomNames = new ArrayList<String>();
						player.getRaids().roomPaths= new ArrayList<Location>();
						player.getRaids().currentRoom = 0;
						...
						player.getRaids().leftHand = false;
						player.raidPoints+=3;
							player.sendMessage("@red@You receive 3 points from finishing this raid");
							player.sendMessage("@red@You now have "+player.raidPoints+" points.");
