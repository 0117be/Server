...
public class FightCave {
		...
		if (Misc.random(100) == 18) {
			 if (player.getItems().getItemCount(13225, true) == 0 && player.summonId != 13225) {
				 PlayerHandler.executeGlobalMessage("[@red@PET@bla@] @cr20@<col=255> " + player.playerName + "</col> received a pet from <col=255>TzTok-Jad</col>.");
					DiscordMessager.sendPetsMessage("```[Pet Manager]" + player.playerName + " has received a pet from Tztok-Jad! Congratulations!```");
	...
	public void gambleInferno() {
		...
		if (Misc.random(100) == 18) {
			 if (player.getItems().getItemCount(13225, true) == 0 && player.summonId != 30951 && player.summonId != 30950) {
				 PlayerHandler.executeGlobalMessage("[@red@PET@bla@] @cr20@<col=255> " + player.playerName + "</col> received a pet from <col=255>TzTok-Zek</col>.");
					DiscordMessager.sendPetsMessage("```[Pet Manager]" + player.playerName + " has received a pet from Tztok-Zek! Congratulations!```");
				 player.getItems().addItemUnderAnyCircumstance(30951, 1);
				 player.getDH().sendDialogues(74, 2180);
			 }
		} else {
			player.getDH().sendDialogues(73, 2180);
