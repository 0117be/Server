 ...
 */
public class CoxBox extends CycleEvent {

    /**
     * The item id of the mystery box required to trigger the event
     */
    private static final int MYSTERY_BOX = 19903;
                    ...
                    if(gift_item == item)
                        if(gift.getKey() == Rarity.UNCOMMON)
                            PlayerHandler.executeGlobalMessage(
                                    "<shad=7000>[<col=16A625>Cox Crate</col>] @cr31@ <col=16A625>" + Misc.capitalize(player.playerName) + " received a rare item: "
                                            + (item.getAmount() > 1 ? (item.getAmount() + "x ") : ItemAssistant.getItemName(item.getId()) + " from a Cox Crate."));
        }
        container.stop();
    }
