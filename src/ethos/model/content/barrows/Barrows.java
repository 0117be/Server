...
public class Barrows {
	...
	*/
	...
	public void useChest() {
					if (!player.getItems().playerHasItem(3451, 1)) {
						player.sendMessage("You need a barrows key to open this chest.");
						return;
					
				} else {
					player.getItems().deleteItem(3451, 1);
					RewardList rewardList = new RewardList();
					brothers.stream().filter(Brother::isDefeated).forEach(br -> rewardList.addAll(br.getRewards()));
					player.setBarrowsChestCounter(player.getBarrowsChestCounter() + 1);
					player.sendMessage("@blu@You have opened "+ player.getBarrowsChestCounter() + " barrows chests.");
					for (int i = 0; i < 12; i++) {
						player.getPA().itemOnInterface(-1, -1, 64503, i);
					}
					giveRewards(rewardList);
	...
	private void giveRewards(RewardList rewardList) {
		ArrayList<RewardItem> randomRewards = getWeightedRewards(rewardList, 3);
