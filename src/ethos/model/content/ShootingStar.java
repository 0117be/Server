...
public class ShootingStar {
	...
	static final int STAR_ID = 30013;
	...
	public static void spawnStar() {
		...
		//	PlayerHandler.getPlayers().forEach(p ->p.getPA().sendFrame126("@or1@ -Crashed Star: @gre@None", 10410));
			if(timer.elapsed(TIME)) {
				...
				PlayerHandler.executeGlobalMessage("@pur@A meat tree has spawned at " +locationData.clue+ "!");
				PlayerHandler.getPlayers().forEach(p -> p.getPA().sendFrame126("@blu@Meat Tree: @gre@"+ShootingStar.CRASHED_STAR.getStarLocation().playerPanelFrame+"", 10410));
				timer.reset();
			}
		} else {
		...
		if(CRASHED_STAR != null) {
			for(Player p : PlayerHandler.getPlayers()) {
				...
				if(CRASHED_STAR.starObject != null) {
					p.getSkilling().stop();
					p.sendMessage("@pur@The meat tree has been chopped down.");
				}
			}
			PlayerHandler.executeGlobalMessage("@pur@The meat tree will spawn again in an hour!");
