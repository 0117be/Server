...
public enum SkillcapePerks {
		...
		SARADOMINI_MAX_CAPE(new int[][] { { 21776 } }),
		ZAMORAKI_MAX_CAPE(new int[][] { { 21780 } }),
		GUTHIXI_MAX_CAPE(new int[][] { { 21784 } }),
		GUTHIX_MAX_CAPE(new int[][] { { 13335 } 
	});
	
	public static final int[] MAX_CAPE_IDS = { 13280, 13329, 13337, 13331, 13333, 13335, 20760, 21285, 21776, 21780, 21784, 21898 };
	
	public static final SkillcapePerks[] MAX_CAPES = { MAX_CAPE, ARDOUGNE_MAX_CAPE, FIRE_MAX_CAPE, AVAS_MAX_CAPE, SARADOMIN_MAX_CAPE, ZAMORAK_MAX_CAPE, GUTHIXI_MAX_CAPE, SARADOMINI_MAX_CAPE, ZAMORAKI_MAX_CAPE, INFERNAL_MAX_CAPE, ASSEMBLER_MAX_CAPE, GUTHIX_MAX_CAPE };
	...
	public static boolean isWearingMaxCape(Player player) {
		if (MAX_CAPE.isWearing(player) || 
			...
			SARADOMINI_MAX_CAPE.isWearing(player) || 
			ZAMORAKI_MAX_CAPE.isWearing(player) || 
			GUTHIXI_MAX_CAPE.isWearing(player) || 
	...
	public static void mixCape(Player player, String cape) {
		...
		case "SARADOMINI":
			if (!player.getItems().playerHasItem(21791)) {
				player.sendMessage("You must have an Imbued saradomin cape in order to do this.");
				return;
			}
		...
		case "ZAMORAKI":
			if (!player.getItems().playerHasItem(21795)) {
				player.sendMessage("You must have an Imbued Zamorak cape in order to do this.");
				return;
			}
		...
		case "GUTHIXI":
			if (!player.getItems().playerHasItem(21793)) {
				player.sendMessage("You must have an Imbued Guthix cape in order to do this.");
				return;
			}
		...
		case "ASSEMBLER":
			if (!player.getItems().playerHasItem(22109)) {
				player.sendMessage("You must have an Ava's Assembler in order to do this.");
				return;
			}
			player.getItems().deleteItem(22109, 1);
			player.getItems().addItem(21898, 1);
			player.getItems().addItem(21900, 1);
			player.getDH().sendItemStatement("You've combined the assembler with your max cape.", 21898);
			break;
		...
		case "INFERNAL":
			...
			player.getDH().sendItemStatement("You've combined the Infernal Cape with your Max Cape.", 21285);
			break;
