...
public class Zulrah {
	...
	public void stop() {
		...
		if (globalBest.getFirst() == null || globalBest.getSecond() == null || time < globalBest.getSecond() && globalBest.getSecond() != 0) {
			...
			DiscordMessager.sendAnnouncement("```[Zulrah]" + Misc.capitalize(player.playerName)
			+ " set the record for best time against zulrah with " + duration + ".```");
			if (globalBest.getFirst() != null && globalBest.getSecond() != null) {
				...
				DiscordMessager.sendAnnouncement("```[Zulrah] The old record was set by " + globalBest.getFirst()
				+ " with a time of " + Misc.toFormattedMS(globalBest.getSecond()) + ".```");
			}
