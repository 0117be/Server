...
public class Potions {
	...
	public void handlePotion(int itemId, int slot) {
			if (itemId >= 20913 && itemId <= 20992) {
			    if (Boundary.isIn(c, Boundary.ENTIRERAIDS) || Boundary.isIn(c, Boundary.TOB) || Boundary.isIn(c, Boundary.RAIDS) || Boundary.isIn(c, Boundary.OLM)) {
			    	if (itemId == 20989) {
			    		doRaidOverload(itemId, -1, slot);
			        ...
			        doRaidOverload(itemId, itemId -1, slot);
			        System.out.println("should be drinking");
			    } else {
			    c.sendMessage("I should not attempt to drink these outside raid dungeons.");
			    }
			}
		...
		if (c.potionTimer.elapsed() > 1200) {
			...
			switch (itemId) {
			/**
			 * Start of Raid potions
			 */
			case 20980:
				drinkSaradominBrew(itemId, 20979, slot); // xerics
				break;
			case 20979:
				drinkSaradominBrew(itemId, 20978, slot); // xerics
				break;
			case 20978:
				drinkSaradominBrew(itemId, 20977, slot); // xerics
				break;
			case 20977:
				drinkSaradominBrew(itemId, 20800, slot); // xerics
				break;
			case 20944:
				drinkMagicPotion(itemId, 20943, slot, 6, false); // Kodai Potion
				break;
			case 20943:
				drinkMagicPotion(itemId, 20942, slot, 6, false);
				break;
			case 20942:
				drinkMagicPotion(itemId, 20941, slot, 6, false);
				break;
			case 20941:
				drinkMagicPotion(itemId, 20800, slot, 6, false);
				break;
			case 20932:
				drinkStatPotion(itemId, 20931, slot, 4, false); // twisted potion
				break;
			case 20931:
				drinkStatPotion(itemId, 20930, slot, 4, false);
				break;
			case 20930:
				drinkStatPotion(itemId, 20929, slot, 4, false);
				break;
			case 20929:
				drinkStatPotion(itemId, 20800, slot, 4, false);
				break;
			case 20956:
				drinkSuperRestorePot(itemId, 20955, slot); // revitalisation
				break;
			case 20955:
				drinkSuperRestorePot(itemId, 20954, slot); // revit
				break;
			case 20954:
				drinkSuperRestorePot(itemId, 20953, slot); // revit
				break;
			case 20953:
				drinkSuperRestorePot(itemId, 20800, slot); // revit
				break;
			case 20920:
				drinkStatPotion(itemId, 20919, slot, 0, true); // elder pot
				enchanceStat(1, true);
				enchanceStat(2, true);
				break;
			case 20919:
				drinkStatPotion(itemId, 20918, slot, 0, true); // elder
				enchanceStat(1, true);
				enchanceStat(2, true);
				break;
			case 20918:
				drinkStatPotion(itemId, 20917, slot, 0, true); // elder
				enchanceStat(1, true);
				enchanceStat(2, true);
				break;
			case 20917:
				drinkStatPotion(itemId, 20800, slot, 0, true); // elder
				enchanceStat(1, true);
				enchanceStat(2, true);
				break;
			/**
			...
			case 12911:
				...
				break;
			case 12913:
				drinkAntiVenom(12915, slot, 1500);
				break;
			case 12915:
				drinkAntiVenom(12917, slot, 1500);
				break;
			case 12917:
				drinkAntiVenom(12919, slot, 1500);
				break;
			case 12919:
				drinkAntiVenom(-1, slot, 1500);
				c.sendMessage("You quickly smash the empty vial using the trick a Barbarian taught you.");
				break;
			case 12625:
	...
	public void doOverload(int itemId, int replaceItem, int slot) {
		...
		if (c.inWild()) {
			c.sendMessage("@red@Overloads do not work in the wilderness.");
			return;
		}
	...
	public void doRaidOverload(int itemId, int replaceItem, int slot) {
		int health = c.getHealth().getAmount();
		if (health <= 50) {
			c.sendMessage("I should get some more lifepoints before using this!");
			return;
		...
		c.getPA().sendGameTimer(ClientGameTimer.OVERLOAD, TimeUnit.MINUTES, 5);
		c.hasRaidOverloadBoost = false;
		c.startAnimation(829);
		c.playerItems[slot] = replaceItem + 1;
		c.getItems().resetItems(3214);
		c.hasRaidOverloadBoost = true;
		createOverloadDamageEvent();
		doRaidOverloadBoost();
		handleOverloadTimers();
		c.getPA().refreshSkill(0);
	...
	public void doRaidOverloadBoost() {
		int[] toIncrease = { 0, 1, 2, 4, 6 };
		int boost;
		for (int i = 0; i < toIncrease.length; i++) {
			boost = (int) (getRaidOverloadBoost(toIncrease[i]));
			c.playerLevel[toIncrease[i]] += boost;
			if (c.playerLevel[toIncrease[i]] > (c.getLevelForXP(c.playerXP[toIncrease[i]]) + boost))
				c.playerLevel[toIncrease[i]] = (c.getLevelForXP(c.playerXP[toIncrease[i]]) + boost);
			c.getPA().refreshSkill(toIncrease[i]);
	...
	public double getRaidOverloadBoost(int skill) {
		double boost = 1;
		switch (skill) {
		case 0:
		case 1:
		case 2:
			boost = 14 + (c.getLevelForXP(c.playerXP[skill]) * .25);
			break;
		case 4:
			boost = 14 + (c.getLevelForXP(c.playerXP[skill]) * .25);
			break;
