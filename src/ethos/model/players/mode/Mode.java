...
public abstract class Mode {
	...
	private static final List<Mode> MODES = ImmutableList
			.copyOf(Arrays.asList(new RegularMode(ModeType.REGULAR), new IronmanMode(ModeType.IRON_MAN), new Hardcore(ModeType.HARDCORE), new UltimateIronmanMode(ModeType.ULTIMATE_IRON_MAN), new BrnzMode(ModeType.BRNZ), new OsrsMode(ModeType.OSRS),new GroupIronman(ModeType.GROUP_IRONMAN)));
	...
	public boolean isBrnz() {
		return type == ModeType.BRNZ;
	}
	
	/**
	 * Determines if the {@link #type} is equal to {@link ModeType#Hardcore Ironman}
	 * 
	 * @return {@code true} if the player is of this type, otherwise {@code false}
	 */
