	...
	public static void calculateCombatDamage(Player attacker, NPC defender, CombatType combatType, Special special) {
		...
		if (Objects.nonNull(attacker) && Objects.nonNull(defender)) {
			if (combatType.equals(CombatType.MELEE)) {
				...
				if (attacker.getItems().getItemCount(30014, true) == 1 && combatType.equals(CombatType.RANGE) ||attacker.getItems().getItemCount(30014, true) == 1 && combatType.equals(CombatType.MELEE) || attacker.getItems().getItemCount(30014, true) == 1 && combatType.equals(CombatType.MAGE)) {
					if (special != null && (NPCHandler.npcs != null)) {
				...
				case 5416:
					if (combatType.equals(CombatType.MELEE)) {
						...
						attacker.forcedChat("Max Melee hit: "+maximumDamage);
						attacker.sendMessage("Max Melee hit: "+maximumDamage);
					}
					if (combatType.equals(CombatType.RANGE)) {
						damage = maximumDamage;
						attacker.stopMovement();
						attacker.getCombat().resetPlayerAttack();
						attacker.forcedChat("Max Range hit: "+maximumDamage);
					...
					if (combatType.equals(CombatType.MAGE)) {
						...
						attacker.stopMovement();
						attacker.getCombat().resetPlayerAttack();
						attacker.forcedChat("Max Magic hit: "+maximumDamage);
				...
				if (attacker.getItems().isWearingItem(7806, attacker.playerWeapon) || attacker.getItems().isWearingItem(7807, attacker.playerWeapon) || attacker.getItems().isWearingItem(7808, attacker.playerWeapon) || attacker.getItems().isWearingItem(7809, attacker.playerWeapon)) {
					if (defender != null && defender.npcType == 1698 || defender.npcType == 6355 || defender.npcType == 6332 || defender.npcType == 1698 || defender.npcType == 3136) {
						accuracy = 100000;
					}
				}
				...
				if (attacker.getItems().isWearingItem(32099)) {	
					if (damage > 0) {
						attacker.getHealth().increase(damage);
					}
				}
				...
				if (attacker.magicDef) {
					if (defender.npcType != 7413) {
						attacker.getPA().addSkillXP((damage * (attacker.getMode().getType().equals(ModeType.OSRS) ? 20 : Config.MELEE_EXP_RATE) / 3), 1, true);
...
//				break;
			...
			if (damage.getCombatType() != null) {
				...
				case RANGE:
					...
					if (c.lastWeaponUsed >= 4212 || c.lastWeaponUsed <= 4223 ||  c.lastWeaponUsed == 22550 || c.lastWeaponUsed <= 4223 ||  c.lastWeaponUsed == 25867 || c.lastWeaponUsed == 11749 || Item.getItemName(c.playerEquipment[3]).contains("crystal bow")) {
						dropArrows = false;
					}
	...
	public static void attackNpc(Player c, int i) {
		...
		if (NPCHandler.npcs[i] != null) {
			...
			if (NPCHandler.npcs[i].npcType == 1046) {
				...
				if (c.playerEquipment[c.playerHat] != 32103) {
					c.sendMessage("You need to be wearing the Snow Santa hat to attack him!.");
					c.npcIndex = 0;
					return;
				}
			}
			...
			if (c.attackTimer <= 0) {
				...
				case 21816:
					...
					if (c.getEtherCharge() <= 0) {
						c.setEtherCharge(0);
						c.sendMessage("@pur@Your bracelet has lost all charge.");
						c.getItems().deleteItem(21816, 1, c.playerHands);
						c.getItems().wearItem(21817, 1, c.playerHands);
						break;
				}
				}
				...
				if(NPCHandler.npcs[i].npcType == 7888 || NPCHandler.npcs[i].npcType == 7884 ){
					
					distance = 2;
				}
				if(distanceToNpc > 10 && NPCHandler.npcs[i].npcType == 7888 ){
					c.sendMessage("@red@Your weapons are useless at this range against Dusk, move closer.");
					c.stopMovement();
					c.getCombat().resetPlayerAttack();
					return;
				}
				if(distanceToNpc > 2 && NPCHandler.npcs[i].npcType == 5416 ){
					c.sendMessage("@red@Please move closer to the combat stone.");
					c.stopMovement();
					c.getCombat().resetPlayerAttack();
					return;
				}
				if(distanceToNpc > 10 && NPCHandler.npcs[i].npcType == 7884 ){
					c.sendMessage("@red@Your weapons are useless at this range against Dawn, move closer.");
					c.stopMovement();
					c.getCombat().resetPlayerAttack();
					return;
				}
				if(NPCHandler.npcs[i].npcType == 3162 && c.usingBow || c.usingMagic || c.autocasting || c.playerEquipment[c.playerWeapon] == 11907 || c.playerEquipment[c.playerWeapon] == 12899){
					distance = 1;
				}
