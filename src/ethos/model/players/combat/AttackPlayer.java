...
public class AttackPlayer {
	...
	public static void applyPlayerMeleeDamage(Player c, int i, int damageMask, int damage, Hitmark hitmark) {
		...
		case 2:
			if (damage > 0) {
				if (o.freezeTimer <= 0)
					o.freezeTimer = 10;
				o.getPA().sendGameTimer(ClientGameTimer.FROZEN, TimeUnit.SECONDS, 10);
				o.gfx0(369);
	...
	public static void addCombatXP(Player c, CombatType type, int damage) {
		...
		case RANGE:
			skills.add(Skill.HITPOINTS.getId());
			experience += (damage * (c.getMode().getType().equals(ModeType.OSRS) ? 20 : Config.RANGE_EXP_RATE)) / 3;
			c.getPA().addSkillXP((damage * (c.getMode().getType().equals(ModeType.OSRS) ? 20 : Config.RANGE_EXP_RATE)) / 3, 3, false);
			switch (c.fightMode) {
			case 0: // Accurate
			case 2: // Rapid
				skills.add(Skill.RANGED.getId());
				experience += (damage * (c.getMode().getType().equals(ModeType.OSRS) ? 20 : Config.RANGE_EXP_RATE));
				c.getPA().addSkillXP(damage * (c.getMode().getType().equals(ModeType.OSRS) ? 20 : Config.RANGE_EXP_RATE), 4, false);
				break;
			case 1: // Block
			case 3:
				skills.add(Skill.RANGED.getId());
				skills.add(Skill.DEFENCE.getId());
				experience += (damage * (c.getMode().getType().equals(ModeType.OSRS) ? 20 : Config.RANGE_EXP_RATE)) + (damage * (c.getMode().getType().equals(ModeType.OSRS) ? 16 : Config.RANGE_EXP_RATE) / 2);
				c.getPA().addSkillXP(damage * (c.getMode().getType().equals(ModeType.OSRS) ? 20 : Config.RANGE_EXP_RATE / 2), 1, false);
				c.getPA().addSkillXP(damage * (c.getMode().getType().equals(ModeType.OSRS) ? 20 : Config.RANGE_EXP_RATE) / 2, 4, false);
				break;
			}
			break;
		case MAGE:
			...
			experience += ((damage * (c.getMode().getType().equals(ModeType.OSRS) ? 20 : Config.MAGIC_EXP_RATE)) / 3) + (damage * (c.getMode().getType().equals(ModeType.OSRS) ? 16 : Config.MAGIC_EXP_RATE));
			c.getPA().addSkillXP((damage * (c.getMode().getType().equals(ModeType.OSRS) ? 20 : Config.MAGIC_EXP_RATE)) / 3, 3, false);
			c.getPA().addSkillXP(damage * (c.getMode().getType().equals(ModeType.OSRS) ? 20 : Config.MAGIC_EXP_RATE), 6, false);
			break;
		}
		if (experience > 0) {
	...
	public static void playerDelayedHit(final Player c, final int i, final Damage d) {
		if (PlayerHandler.players[i] != null) {
			...
			if (!c.usingMagic && c.projectileStage > 0) { // range hit damage
				...
				if (c.lastWeaponUsed >= 4212 || c.lastWeaponUsed <= 4223 ||  c.lastWeaponUsed == 22550 || c.lastWeaponUsed <= 4223 ||  c.lastWeaponUsed == 25867 || c.lastWeaponUsed == 17749 || Item.getItemName(c.playerEquipment[3]).contains("crystal bow")) {
					dropArrows = false;
				}
		...
		if (Objects.nonNull(attacker) && Objects.nonNull(defender)) {
			if (combatType.equals(CombatType.MELEE)) {
				...
				if (freezeDelay > 0 && defender.freezeTimer <= -3 && !attacker.magicFailed) {
					...
					defender.getPA().sendGameTimer(ClientGameTimer.FROZEN, TimeUnit.SECONDS, 10);
				}
				if (attacker.magicDef) {
					attacker.getPA().addSkillXP((damage * (attacker.getMode().getType().equals(ModeType.OSRS) ? 20 : Config.MELEE_EXP_RATE) / 3), 1, false);
				...
				if (c.attackTimer <= 0) {
					...
					if (c.autocasting) {
						...
						if (MagicData.MAGIC_SPELLS[c.spellId][0] >= 12861 && MagicData.MAGIC_SPELLS[c.spellId][0] <= 13023) {
							if (c.playerEquipment[c.playerWeapon] != 4675 && c.playerEquipment[c.playerWeapon] != 22647 && c.playerEquipment[c.playerWeapon] != 6914 && c.playerEquipment[c.playerWeapon] != 21006 && c.playerEquipment[c.playerWeapon] != 12904) {
					...
					boolean projectile = c.usingOtherRangeWeapons || c.usingBow || c.usingMagic || c.getCombat().usingCrystalBow()  || c.getCombat().usingCrawsBow() || c.usingMelee;
					...
					if (c.usingBow && !c.usingOtherRangeWeapons && !c.usingMagic || c.usingCross || c.usingBallista) { // range
																									// hit
																									// delay
						if (c.playerEquipment[c.playerWeapon] == 22550 || c.playerEquipment[c.playerWeapon] == 11749) {
