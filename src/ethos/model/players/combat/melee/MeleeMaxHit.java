...
public class MeleeMaxHit {
	...
	public static double calculateBaseDamage(Player c, boolean special) {
		...
		if (c.npcIndex > 0) {
			...
			if (npc != null) {
				if (c.getSlayer().getTask().isPresent()) {
					if (npc != null && c.getSlayer().getTask().get().matches(npc.getDefinition().getNpcName()) || npc.npcType == 7413) {
						...
						if ((!c.getItems().isWearingItem(4081) || !c.getItems().isWearingItem(12018) || !c.getItems().isWearingItem(10588) || !c.getItems().isWearingItem(12017)) && SLAYER_HELM || c.getItems().isWearingItem(8901)) {
							...
							base *= 1.5;
							if (c.debugMessage)
							c.sendMessage("Damage on Slayer: "+ base +"");
						}
					}
				}
				if (c.getItems().isWearingItem(4081, c.playerAmulet) || c.getItems().isWearingItem(12017, c.playerAmulet)) {
					if (Misc.linearSearch(Config.UNDEAD_IDS, npc.npcType) != -1) {
						base *= 1.15;
						if (c.debugMessage)
							c.sendMessage("Damage on Undead: "+ base +"");
					}
				}
				if (c.getItems().isWearingItem(22557, c.playerAmulet)) {
					if (Misc.linearSearch(Config.REV_NPC, npc.npcType) != -1) {
						base *= 1.5;
						if (c.debugMessage)
							c.sendMessage("Damage on Revs: "+ base +"");
					}
				}
				if (c.getItems().isWearingItem(11824, c.playerWeapon)) {
					if (npc != null && npc.npcType == 319) {
						base *= 9.0;
						if (c.debugMessage)
							c.sendMessage("Damage on Corp: "+ base +"");
					}
				}
				if (c.getItems().isWearingItem(7806, c.playerWeapon) || c.getItems().isWearingItem(7807, c.playerWeapon) || c.getItems().isWearingItem(7808, c.playerWeapon) || c.getItems().isWearingItem(7809, c.playerWeapon)) {
					if (npc != null && npc.npcType == 1698 || npc.npcType == 6355 || npc.npcType == 6332 || npc.npcType == 1698 || npc.npcType == 3136) {
						base *= 2.0;
						if (c.debugMessage)
							c.sendMessage("Damage on Blood: "+ base +"");
					}
				}

				if (c.getItems().isWearingItem(11889, c.playerWeapon)) {
					if (npc != null && npc.npcType == 5862) {
						base *= 1.5;
						if (c.debugMessage)
							c.sendMessage("Damage on Whatchamacllit: "+ base +"");
					}
				}
				//start of perks
				if (c.getItems().getItemCount(10943, true) == 1) {	
					if (c.npcIndex > 0)
							base *= 1.1;												
					}
				...
				if (c.getItems().isWearingItem(10588, c.playerAmulet) || c.getItems().isWearingItem(12018, c.playerAmulet)) {
					if (Misc.linearSearch(Config.UNDEAD_IDS, npc.npcType) != -1) {
						base *= 1.20;
						if (c.debugMessage)
						c.sendMessage("Damage on Undead: "+ base +"");
				...
				if (c.lastWeaponUsed == 20727 && npc.weakToLeafBladedAxe()) {
					base *= 1.175;
					if (c.debugMessage)
					c.sendMessage("Damage with Leafbladed: "+ base +"");
				}
				...
				if (c.playerEquipment[c.playerWeapon] == 22545) {
					base *= 1.5;
					if (c.debugMessage)
					c.sendMessage("Damage with Viggora's Chainmace: "+ base +"");
				}
				}
				if (c.getItems().isWearingItem(19675, c.playerWeapon) && c.getArcLightCharge() > 0) {
					if (Misc.linearSearch(Config.DEMON_IDS, npc.npcType) != -1) {
						base *= 1.90;
						if (c.debugMessage)
							c.sendMessage("Damage on demons: "+ base +"");

					}
				}
		
		if (c.getItems().isWearingItem(22978, c.playerWeapon)) {
			if (Misc.linearSearch(Config.DRAGON_IDS, npc.npcType) != -1 || npc.npcType == 7413 || npc.npcType == 8621) {
				base *= 1.50;
				if (c.debugMessage)
				c.sendMessage("Damage dragons: "+ base +"");
			}
		...
		if (EquipmentSet.DHAROK.isWearingBarrows(c)) {
			...
			if (c.debugMessage)
				c.sendMessage("Damage in Dharok: "+ base +"");
		}
		if (hasObsidianEffect(c) || c.fullVoidMelee()) {
			base = (base * 1.10);
			if (c.debugMessage)
				c.sendMessage("Damage in Void: "+ base +"");
		}
		if (c.prayerActive[1]) {
	...
	public static int calculateMeleeAttack(Player c) {
		...
		if (c.playerEquipment[c.playerWeapon] == 4153) {
			attackLevel -= c.getLevelForXP(c.playerXP[c.playerAttack]) * 0.15;
			if (c.debugMessage)
				c.sendMessage("Attack level with maul: "+ attackLevel +"");
		}
		if (c.playerEquipment[c.playerWeapon] == 4718 && c.playerEquipment[c.playerHat] == 4716 && c.playerEquipment[c.playerChest] == 4720
			...
			attackLevel -= c.getLevelForXP(c.playerXP[c.playerAttack]) * 0.15;
			if (c.debugMessage)
				c.sendMessage("Attack level in dharoks: "+ attackLevel +"");
		}
		if (c.fullVoidMelee()) {
			attackLevel += c.getLevelForXP(c.playerXP[c.playerAttack]) * 0.10;
			if (c.debugMessage)
				c.sendMessage("Attack level in void: "+ attackLevel +"");
		}
		if (c.debugMessage)
			c.sendMessage("Attack level: "+ attackLevel +"");
		if (c.getItems().isWearingItem(19675, c.playerWeapon) && c.getArcLightCharge() > 0) {
			if (c.debugMessage)
				c.sendMessage("Normal Attack Level: "+ attackLevel +"");
			NPC npc = NPCHandler.npcs[c.npcIndex];
			if (Misc.linearSearch(Config.DEMON_IDS, npc.npcType) != -1) {
				attackLevel += c.getLevelForXP(c.playerXP[c.playerAttack]) * 0.70;
				if (c.debugMessage)
					c.sendMessage("Attack on demon: "+ attackLevel +"");
			}
		}
		if (c.getItems().isWearingItem(22978, c.playerWeapon)) {
			if (c.debugMessage)
			...
			if (Misc.linearSearch(Config.DRAGON_IDS, npc.npcType) != -1) {
				...
				if (c.debugMessage)
					c.sendMessage("Attack on Dragons: "+ attackLevel +"");
			}
		}
		if (c.npcIndex > 0 && c.getSlayer().getTask().isPresent()) {
			...
			if (npc != null && c.getSlayer().getTask().get().matches(npc.getDefinition().getNpcName())) {
				boolean SLAYER_HELM = IntStream.of(c.SLAYER_HELMETS).anyMatch(i -> c.getItems().isWearingItem(i));
				if (!c.getItems().isWearingItem(4081) || !c.getItems().isWearingItem(12018) || !c.getItems().isWearingItem(10588) || !c.getItems().isWearingItem(12017) && SLAYER_HELM || c.getItems().isWearingItem(8901)) {
					attackLevel += c.getLevelForXP(c.playerXP[c.playerAttack]) * 0.1667;
					if (c.debugMessage)
						c.sendMessage("Attack on Slayer Task: "+ attackLevel +"");
				}
			}
		}
		if (c.npcIndex > 0) {
			...
			if (c.getItems().isWearingItem(4081, c.playerAmulet) || c.getItems().isWearingItem(12017, c.playerAmulet)) {
				if (Misc.linearSearch(Config.UNDEAD_IDS, npc.npcType) != -1) {
					attackLevel += c.getLevelForXP(c.playerXP[c.playerAttack]) * 0.15;
					c.sendMessage("Attack on Undead: "+ attackLevel +"");
				}
			}
			if (c.getItems().isWearingItem(12018, c.playerAmulet) || c.getItems().isWearingItem(10588, c.playerAmulet)) {
				if (Misc.linearSearch(Config.UNDEAD_IDS, npc.npcType) != -1) {
					attackLevel += c.getLevelForXP(c.playerXP[c.playerAttack]) * 0.20;
					if (c.debugMessage)
						c.sendMessage("Attack on Undead: "+ attackLevel +"");
