...
import ethos.model.content.bonus.DoubleMinigames;
import ethos.model.content.bonus.DoubleRaidSaturday;
import ethos.model.content.bonus.DoubleSlayerPoints;
...
import ethos.model.content.teleportation.QuickSetsTab;
...
import ethos.model.minigames.raids.Tob;
...
import ethos.model.players.skills.agility.impl.WerewolfAgility;
import ethos.model.players.skills.agility.impl.WildernessAgility;
import ethos.model.players.skills.agility.impl.rooftop.RooftopAlkharid;
import ethos.model.players.skills.agility.impl.rooftop.RooftopArdougne;
import ethos.model.players.skills.agility.impl.rooftop.RooftopCanifis;
//import ethos.model.players.skills.agility.impl.rooftop.RooftopDraynor;
import ethos.model.players.skills.agility.impl.rooftop.RooftopFalador;
import ethos.model.players.skills.agility.impl.rooftop.RooftopFalador1;
import ethos.model.players.skills.agility.impl.rooftop.RooftopRelleka;
...
public class Player extends Entity {
	...
	public int RaidoverloadTimer;
	public boolean RaidoverloadBoosted;
	...
	public boolean tradeAlternator;
	public boolean opponentTradeAlternator;
	...
	private MegaBox megaBox= new MegaBox(this);
	
	public MegaBox getMegaBox() {
		return megaBox;
	}
	...
	private XmasPresent XmasPresent= new XmasPresent(this);

	public XmasPresent getXmasPresent() {
		return XmasPresent;
	}
	...
	private ScratchTicket ScratchTicket = new ScratchTicket(this);
	private TobBox tobbox = new TobBox(this);
	private CoxBox coxbox = new CoxBox(this);
	private JarofSouls jarofSouls = new JarofSouls(this);
	private CarePackage carepackage = new CarePackage(this);
	private JarofSwamp jarofSwamp = new JarofSwamp(this);
	private XmasPresentBlue XmasPresentBlue = new XmasPresentBlue(this);
	private XmasPresentWhite XmasPresentWhite = new XmasPresentWhite(this);
	private JarofDirt jarofDirt = new JarofDirt(this);
	private MeatTree meattree = new MeatTree(this);
	private JarofSand jarofSand = new JarofSand(this);
	...
	private QuickSetsTab handleSet = new QuickSetsTab(this);
	...
	private WerewolfAgility werewolfAgility = new WerewolfAgility();
	...
	private RooftopFalador1 rooftopFalador1 = new RooftopFalador1();
	private RooftopRelleka rooftopRelleka = new RooftopRelleka();
	 ...
	 * Integers
	 */
	public int raidPoints, lastSangHit, unfPotHerb, unfPotAmount, wrenchObject = 0, halloweenOrderNumber = 0, speed1 = -1,
			speed2 = -1, safeBoxSlots = 15, hostAmount = 3, corpDamage = 0, direction = -1, dialogueOptions = 0,
			sireHits = 0, lastMenuChosen = 0, dreamTime, hydraAttackCount = 0, countUntilPoison = 0, unNoteItemId = 0, lootValue = 0, lowMemoryVersion = 0, emote,
			...
			crabsKilled, treesCut, pkp, killcount, deathcount, votePoints, questPoints, dailyPoints, bloodPoints, amDonated, level1 = 0,
			...
			delayedDamage2 = 0, delayedDamage3 = 0, pcPoints = 0, bossPoints = 0,donatorPoints = 0, magePoints = 0, lastArrowUsed = -1, clanId = -1,
	...
	private int chatTextColor = 0, chatTextEffects = 0, dragonfireShieldCharge, runEnergy = 100, lastEnergyRecovery,
			...
			toxicBlowpipeCharge, toxicBlowpipeAmmo, toxicBlowpipeAmmoAmount, serpentineHelmCharge, tridentCharge, sangStaffCharge,
			toxicTridentCharge, arcLightCharge, etherCharge, crawCharge, vigCharge, sceptreCharge, runningDistanceTravelled, trivaPoints, interfaceOpen;
	...
	public String getATTACK_TYPE() {
	   ...
	   public String getLoginNameLower() {
	        return getLoginNameLower().toLowerCase();
	    }
	...
	public boolean teleportingToDistrict = false, morphed = false, isIdle = false, boneOnAltar = false,
			...
			documentAnimation = false, inProcess = false, isStuck = false, isBusyFollow = false, hasOverloadBoost,hasRaidOverloadBoost,
	...
	public long wogwDonationAmount, lastAuthClaim, totalGorillaDamage, totalGorillaHitsDone, totalMissedGorillaHits,
			...
			fightCaveLeaveTimer, infernoLeaveTimer, lastFire, lastMove, bonusXpTime, bonusDropTime, yellDelay, craftingDelay,
	...
	private long revertModeDelay, experienceCounter, bestZulrahTime, lastIncentive, lastOverloadBoost, lastRaidOverloadBoost, nameAsLong,
	...
	public void highscores() {
		getPA().sendFrame126("Revelation - Top PKers Online", 6399); // Title
		for (int i = 0; i < 10; i++) {
			if (ranks[i] > 0) {
	...
	public void destruct() {
		...
		disconnected = true;
		if(!getRights().isOrInherits(Right.OWNER) && !getRights().isOrInherits(Right.MANAGER) && !getRights().isOrInherits(Right.ADMINISTRATOR) && !getRights().isOrInherits(Right.GAME_DEVELOPER)) {
			if(getRights().isOrInherits(Right.ULTIMATE_IRONMAN)) {
				com.everythingrs.hiscores.Hiscores.update("b15O3ET2Fqh5QzTLt31utOzyu06uNMB2ApTFgrBpR8Agzy7mvHgWF89nVbveI1hXHZUU9Qhz", "Ultimate Ironman", this.playerName, this.playerRank, this.playerXP, debugMessage);
			}
			if (getRights().isOrInherits(Right.HARDCORE)) {
				com.everythingrs.hiscores.Hiscores.update("b15O3ET2Fqh5QzTLt31utOzyu06uNMB2ApTFgrBpR8Agzy7mvHgWF89nVbveI1hXHZUU9Qhz", "Hardcore Ironman", this.playerName, this.playerRank, this.playerXP, debugMessage);
			}
			if (getRights().isOrInherits(Right.BRNZ)) {
				com.everythingrs.hiscores.Hiscores.update("b15O3ET2Fqh5QzTLt31utOzyu06uNMB2ApTFgrBpR8Agzy7mvHgWF89nVbveI1hXHZUU9Qhz", "Bronze Mode", this.playerName, this.playerRank, this.playerXP, debugMessage);
			}
			if (getRights().isOrInherits(Right.OSRS)) {
					com.everythingrs.hiscores.Hiscores.update("b15O3ET2Fqh5QzTLt31utOzyu06uNMB2ApTFgrBpR8Agzy7mvHgWF89nVbveI1hXHZUU9Qhz", "Extreme Mode", this.playerName, this.playerRank, this.playerXP, debugMessage);
			}
			if(getRights().isOrInherits(Right.IRONMAN)) {
					com.everythingrs.hiscores.Hiscores.update("b15O3ET2Fqh5QzTLt31utOzyu06uNMB2ApTFgrBpR8Agzy7mvHgWF89nVbveI1hXHZUU9Qhz", "Ironman", this.playerName, this.playerRank, this.playerXP, debugMessage);
			} else {			
				com.everythingrs.hiscores.Hiscores.update("b15O3ET2Fqh5QzTLt31utOzyu06uNMB2ApTFgrBpR8Agzy7mvHgWF89nVbveI1hXHZUU9Qhz", "Normal Mode", this.playerName, this.playerRank, this.playerXP, debugMessage);
			}
		}
    ...
    public void quest(String quest, String... line) {
        ...
        getPA().showInterface(12140);
        getPA().sendFrame126("You have completed " + quest + ".", 12144);
        getPA(). sendFrame126("" + questPoints +" ", 12147);
        for (int i = 0; i < line.length; i++) {
        	getPA().sendFrame126(line[i], 12150 + i);
        }
    }
	...
	private enum RankUpgrade {
		CONTRIBUTOR(Right.CONTRIBUTOR, 10), SPONSOR(Right.SPONSOR, 50), SUPPORTER(Right.SUPPORTER, 100), VIP(
				Right.DONATOR, 250), INFAMOUS(Right.SUPER_DONATOR,
						500), LEGENDARY(Right.EXTREME_DONATOR, 1000), MYTHICAL(Right.LEGENDARY, 2000);
	...
	public void initialize() {
		try {
			 ...
			 * Welcome messages
			 */
			sendMessage("@blu@<shad=7000>Welcome to Revelation!");
			sendMessage("@blu@<shad=7000>----------------------");
			sendMessage("@blu@<shad=7000>Make sure to stay in tune with all the updates on our ::discord!");
			sendMessage("@blu@<shad=7000>We are always open to any suggestions & concerns");
			sendMessage("@blu@<shad=7000>Make sure to check the quest tab, we have daily events!");
			sendMessage("@blu@<shad=7000>Follow all of the rules & enjoy the grind!");
			
			if (DoubleExperience.isDoubleExperience(this))
				sendMessage("@cr10@<Col=6092><shad=6092><col=F8AA36>Bonus Experience Weekend is ACTIVE!");
			if (DoubleSlayerPoints.isDoubleSlayer())
				sendMessage("@cr10@<Col=6092><shad=6092><col=F8AA36> Double Slayer Tuesday is ACTIVE! Double Slayer points from all tasks!");
			if (Config.DOUBLE_VOTE_INCENTIVES)
				sendMessage("<img=10><Col=6092><shad=6092><col=F8AA36>The Staff Team is hosting a Voting Event!"
						+ "Receive 50% more tickets when claiming votes!</col>");
			if (DoubleRaidSaturday.isDoubleRaid())
				sendMessage("@cr10@<Col=6092><shad=6092><col=F8AA36> Double Raid Saturday! Double loot from the Raids Chest!");
			if (DropRateBoost.isDropRateBoost())
				sendMessage("@cr10@<Col=6092><shad=6092><col=F8AA36> Drop boost Thursday! 5% drop rate boost for the whole day!");
			if (DoubleMinigames.isDoubleMinigames())
				sendMessage("@cr10@<Col=6092><shad=6092><col=F8AA36> Bonus point Monday! You receive double bonus points from pest control all day!");
			...
			if (startPack == false) {
				...
				getRights().remove(Right.HARDCORE);
				startPack = true;
			...
			if(getRights().isOrInherits(Right.MODERATOR) || getRights().isOrInherits(Right.HELPER) ||
				getRights().isOrInherits(Right.ADMINISTRATOR) || getRights().isOrInherits(Right.OWNER)|| getRights().isOrInherits(Right.MANAGER)) {
			{
		
						if(!playerName.equalsIgnoreCase(""))
							PlayerHandler.executeGlobalMessage("@red@[Staff]" + " " + playerName + " has logged in!");
	...
	public void updateQuestTab(){
		...
		getPA().sendFrame126("@or1@ Players in Wilderness: @gre@" +  Boundary.entitiesInArea(Boundary.WILDERNESS) ,10408);
		getPA().sendFrame126("@or1@ Player Rank = @gre@" + getRights().getPrimary().toString(), 10409);
		getPA().sendFrame126("@or1@ Daily Task: " + dailyStatus, 10410);
		getPA().sendFrame126("@or1@ Bonus XP: " + xpString, 10411);
		getPA().sendFrame126("@or1@ My KDR: @gre@"+ (double)(this.deathcount == 0 ? this.killcount + this.deathcount : this.killcount/this.deathcount), 10412);
		getPA().sendFrame126("@or1@ Time Played = @gre@"+time, 10413);
		getPA().sendFrame126("@or1@ Total XP: @whi@" + totalXP, 10414);
		getPA().sendFrame126("@or1@ Bank Value: @whi@" + bankValue, 10415);
		getPA().sendFrame126("@or1@ Inventory Value: @whi@" + invValue, 10416);
		if (amDonated >= 10 && amDonated < 50) {
		getPA().sendFrame126("@or1@ Donator Rank: @blu@@cr4@Sapphire Donator", 10417);
		}else if (amDonated >= 50 && amDonated < 100) {
	getPA().sendFrame126("@or1@ Donator Rank: @gre@@cr5@Emerald Donator", 10417);
		}else if (amDonated >= 100 && amDonated < 250) {
	getPA().sendFrame126("@or1@ Donator Rank: @red@@cr6@Ruby Donator", 10417);
		}else if (amDonated >= 250 && amDonated < 500) {
	getPA().sendFrame126("@or1@ Donator Rank: @whi@@cr7@Diamond Donator", 10417);
		}else if (amDonated >= 500 && amDonated < 1000) {
	getPA().sendFrame126("@or1@ Donator Rank: @pur@@cr8@Dragonstone Donator", 10417);
		}else if (amDonated >= 1000 && amDonated < 2000) {
	getPA().sendFrame126("@or1@ Donator Rank: @or1@@cr9@Zenyte Donator", 10417);
		}else if (amDonated >= 2000 && amDonated < 10000) {
	getPA().sendFrame126("@or1@ Donator Rank: @cr16@@red@G@gre@o@yel@d@blu@l@pur@y @blu@Donator", 10417);	
		}else if  (!getRights().isOrInherits(Right.DONATOR)) 
	getPA().sendFrame126("@or1@ Donator Rank: @gre@NONE", 10417);

		getPA().sendFrame126("@or1@ Total Donated:@whi@ $" + this.amDonated, 10418);
		if (DoubleSlayerPoints.isDoubleSlayer()) {
			getPA().sendFrame126("@or1@ Slayer Tuesday: @gre@Double Slayer Pts", 10419);
		}else 	if (DoubleMinigames.isDoubleMinigames()) {
			getPA().sendFrame126("@or1@ PC Monday: @gre@2x PC Pts", 10419);
	}else 	if (DropRateBoost.isDropRateBoost())  {
		getPA().sendFrame126("@or1@ Drop Thursday: @gre@5% DR Boost", 10419);
	} else 	if (DoubleRaidSaturday.isDoubleRaid()) {
		getPA().sendFrame126 ("@or1@ Raid Saturday: @gre@2x Raid Chest", 10419);
		getPA().sendFrame126 ("", 10420);
	} else 	if (Calendar.getInstance().get(Calendar.DAY_OF_WEEK) == Calendar.WEDNESDAY) {
		getPA().sendFrame126 ("@or1@ Wednesday Fun: @gre@2x Boss Points & XP", 10419);
		getPA().sendFrame126 ("", 10420);
	} else 	
		getPA().sendFrame126 ("@or1@ Events: @red@Inactive", 10419);
		getPA().sendFrame126 ("", 10420);
	...
	public void updateRedTab() {
		questTab = 2;
		getPA().sendFrame126("@gre@<shad=7000> "+ Misc.capitalizeJustFirst(this.playerName) + " - Drop Rate Boost "+DropManager.getModifier1(this) + "%.", 10407);
		getPA().sendFrame126("@or1@ Vote Points: @gre@" + this.votePoints, 10408);
		getPA().sendFrame126("@or1@ PK Points = @gre@" + this.pkp, 10409);
		getPA().sendFrame126("@or1@ Slayer Points = @gre@" + this.getSlayer().getPoints(), 10410); //m.getPointReward(0)
		getPA().sendFrame126("@or1@ PC points = @gre@" + this.pcPoints, 10411);
		getPA().sendFrame126("@or1@ Boss Points = @gre@" +this.bossPoints, 10412);
		getPA().sendFrame126("@or1@ Daily Task Points = @gre@"+this.dailyPoints, 10413);
		getPA().sendFrame126("@or1@ Slayer Kills Left = @gre@" + this.getSlayer().getTaskAmount() +" "+ this.getSlayer().getTask().get().getPrimaryName(), 10414);
		getPA().sendFrame126("@or1@ Consecutive Tasks = @gre@" + this.getSlayer().getConsecutiveTasks(), 10415);
		getPA().sendFrame126("@or1@ Bounty Hunter Points = @gre@" + this.getBH().getBounties(), 10416);
		getPA().sendFrame126("@or1@ Raid Points = @gre@" + this.raidPoints, 10417);
		getPA().sendFrame126("@or1@ Achievement Points = @gre@" + this.getAchievements().points, 10418);
	...
	public void updatePurpleTab() {
		questTab = 3;
		getPA().sendFrame126("@or1@ Useful Tab",10407);
		getPA().sendFrame126(" @or2@Open Collection Log", 10408);
		getPA().sendFrame126(" @or2@Open NPC Droptables", 10409);
		getPA().sendFrame126(" @or2@Open Guides", 10410);
		getPA().sendFrame126(" @or2@Open Achievements", 10411);
		getPA().sendFrame126(" @or2@Open Kill Log", 10412);
		getPA().sendFrame126(" @or2@Check Event Boss", 10413);
	...
	public int totaltobFinished;

	public boolean hasClaimedRaidChest;
	public boolean hasClaimedTobChest;

	public int[] SLAYER_HELMETS = { 11864, 11865, 19639, 19641, 19643, 19645, 19647, 19649, 21264, 21266, 21888, 21890, 23073, 23075 };
	public int[] IMBUED_SLAYER_HELMETS = { 11865, 19641, 19645, 19649, 21266, 21890, 23075, 25912 };
	
	public int[] RAID_CAPES = { 22388, 22390, 22392, 22394, 22396 };
	...
	public void process() {
		...
		if (bonusXpTime == 1) {
			sendMessage("@blu@Your time is up. Your XP is no longer boosted by the book.");
		}
		if (dailyTaskDate != DailyTasks.getTodayDate()) {
		...
		if (bonusDropTime == 1) {
			sendMessage("@blu@Your time is up. Your Double Drop bonus has expired.");
		}
		if (respawnTimer == 7) {
			respawnTimer = -6;
			getPA().giveLife();
		...
		if (this.getRaids().isInStorm()) {
			this.appendDamage(2, Hitmark.HIT);
		}
		if (respawnTimer > -6) {
			respawnTimer--;
		}
		...
		if (hasRaidOverloadBoost) {
			if (System.currentTimeMillis() - lastRaidOverloadBoost > 15000) {
				getPotions().doRaidOverloadBoost();
				lastRaidOverloadBoost = System.currentTimeMillis();
			}
		}
		...
		else if (inWild() && Boundary.isIn(this, Boundary.WILDERNESS_PARAMETERS)) {
			int modY = absY > 6400 ? absY - 6400 : absY;
			wildLevel = (((modY - 3520) / 8) + 1);
			if (Config.SINGLE_AND_MULTI_ZONES) {
				getPA().sendFrame126("@yel@Level: " + wildLevel, 199);
		...
		else if (inWild() && Boundary.isIn(this, Boundary.WILDERNESS)) {
			int modY = absY > 6400 ? absY - 6400 : absY;
			wildLevel = (((modY - 3520) / 8) + 1);
			if (Config.SINGLE_AND_MULTI_ZONES) {
				getPA().sendFrame126("@yel@Level: " + wildLevel, 199);
	...
	//	 getPA().walkableInterface(197);
		...
		if(Boundary.isIn(this, Boundary.RISKY)){
			wildLevel=126;
		}
		...
		if (this.skullTimer > 0 && !this.getItems().isWearingItem(22557, playerAmulet)) {
			--skullTimer;
			if (skullTimer == 1) {
				isSkulled = false;
  ...
  //  }
	...
	public RooftopFalador1 getRoofTopFalador1() {
		return rooftopFalador1;
	}
	public RooftopRelleka getRoofTopRelleka() {
		return rooftopRelleka;
	}
...
//	}
	...
	public void updateRank() {
		...
		if (amDonated >= 10 && amDonated < 50) {
			if (getRights().isOrInherits(Right.IRONMAN) || getRights().isOrInherits(Right.ULTIMATE_IRONMAN) || getRights().isOrInherits(Right.HARDCORE) || getRights().isOrInherits(Right.OSRS) || getRights().isOrInherits(Right.BRNZ) || getRights().isOrInherits(Right.HELPER) || getRights().isOrInherits(Right.MODERATOR)) {
		...
		if (amDonated >= 100 && amDonated < 300) {
			if (getRights().isOrInherits(Right.IRONMAN) || getRights().isOrInherits(Right.ULTIMATE_IRONMAN) || getRights().isOrInherits(Right.HARDCORE) || getRights().isOrInherits(Right.OSRS) || getRights().isOrInherits(Right.BRNZ) || getRights().isOrInherits(Right.HELPER) || getRights().isOrInherits(Right.MODERATOR)) {
		...
		if (amDonated >= 500 && amDonated < 1000) {
			if (getRights().isOrInherits(Right.IRONMAN) || getRights().isOrInherits(Right.ULTIMATE_IRONMAN) || getRights().isOrInherits(Right.HARDCORE) || getRights().isOrInherits(Right.OSRS) || getRights().isOrInherits(Right.BRNZ) || getRights().isOrInherits(Right.HELPER) || getRights().isOrInherits(Right.MODERATOR)) {
				...
				//sendMessage("Your hidden sponser donator rank is now active.");
			} else {
				getRights().setPrimary(Right.SUPER_DONATOR);
				//sendMessage("Please relog to receive your sponser donator rank.");
			}
		}
		if (amDonated >= 1000 && amDonated < 2500) {
			if (getRights().isOrInherits(Right.IRONMAN) || getRights().isOrInherits(Right.ULTIMATE_IRONMAN) || getRights().isOrInherits(Right.HARDCORE) || getRights().isOrInherits(Right.OSRS) || getRights().isOrInherits(Right.BRNZ) || getRights().isOrInherits(Right.HELPER) || getRights().isOrInherits(Right.MODERATOR)) {
				getRights().add(Right.EXTREME_DONATOR);
				//sendMessage("Your hidden executive donator rank is now active.");
			} else {
				getRights().setPrimary(Right.EXTREME_DONATOR);
				//sendMessage("Please relog to receive your executive donator rank.");
			}
		}
		if (amDonated >= 2500) {
			if (getRights().isOrInherits(Right.IRONMAN) || getRights().isOrInherits(Right.ULTIMATE_IRONMAN) || getRights().isOrInherits(Right.HARDCORE) || getRights().isOrInherits(Right.OSRS) || getRights().isOrInherits(Right.BRNZ) || getRights().isOrInherits(Right.HELPER) || getRights().isOrInherits(Right.MODERATOR)) {
				getRights().add(Right.LEGENDARY);
				//sendMessage("Your hidden Godly donator rank is now active.");
			} else {
				getRights().setPrimary(Right.LEGENDARY);
				//sendMessage("Please relog to receive your Godly donator rank.");
			}
		}
	...
	public ScratchTicket getScratchTicket() {
		return ScratchTicket;
	}
	public CoxBox getCoxBox() {
		return coxbox;
	}
	public TobBox getTobBox() {
		return tobbox;
	}
	public JarofSouls getJarofSouls() {
		return jarofSouls;
	}
	public CarePackage getCarePackage() {
		return carepackage;
	}
	public JarofDirt getJarofDirt() {
		return jarofDirt;
	}
	
	public JarofSand getJarofSand() {
		return jarofSand;
	}
	public JarofSwamp getJarofSwamp() {
		return jarofSwamp;
	}
	public MeatTree getMeatTree() {
		return meattree;
	}
	public XmasPresentBlue getXmasPresentBlue() {
		return XmasPresentBlue;
	}
	public XmasPresentWhite getXmasPresentWhite() {
		return XmasPresentWhite;
	}
	...
	public final int[] BOWS = { 19481, 19478, 12788, 9185, 11785, 21012, 839, 845, 847, 851, 855, 859, 841, 843, 849,
			...
			4221, 4222, 4223, 4734, 6724, 20997, 22550, 25867, 11749 };
	public final int[] ARROWS = { 9341, 4160, 11959, 10033, 10034, 882, 883, 884, 885, 886, 887, 888, 889, 890, 891,
			...
			9142, 9143, 11875, 21316, 21326, 879, 880, 9335, 9336, 9337, 9338, 9339, 9340, 9341, 9342, 9144, 9145, 9240, 9241, 9242, 9243, 9244, 9245, 9286, 9287, 9288, 9289,
	...
	public final int[] CRYSTAL_BOWS = { 4212, 4214, 4215, 4216, 4217, 4218, 4219, 4220, 4221, 4222, 4223, 11749 };
	...
	public void updateItemsOnDeath() {
		...
		if (Boundary.isIn(this, Boundary.RISKY)) { // what items to keep
			itemAssistant.keepItem(0, false);
			itemAssistant.keepItem(1, false);
			itemAssistant.keepItem(2, false);
		}
		
		if (prayerActive[10] && System.currentTimeMillis() - lastProtItem > 700 && Boundary.isIn(this, Boundary.RISKY)) {
			itemAssistant.keepItem(3, false);
		}
	...
	public boolean inTob() {
		return (absX > 3117 && absX < 3345 && absY > 4191 && absY < 4569);
	}
	...
	public boolean inTobStart() {
		return (absX < 3688 && absX > 3641 && absY < 3233 && absY > 3198);
	...
	public boolean inMulti() {
		if (Boundary.isIn(this, Zulrah.BOUNDARY) || Boundary.isIn(this, Boundary.CORPOREAL_BEAST_LAIR)
				...
				|| Boundary.isIn(this, Boundary.TEKTON)  	|| Boundary.isIn(this, Boundary.GALVEK)|| Boundary.isIn(this, Boundary.SKELETAL_MYSTICS)
				|| Boundary.isIn(this, Boundary.RAIDS) || Boundary.isIn(this, Boundary.OLM)
				|| Boundary.isIn(this, Boundary.ICE_DEMON) || Boundary.isIn(this, Boundary.REV_CAVE)|| Boundary.isIn(this, Boundary.TOB) || Boundary.isIn(this, Boundary.CATACOMBS)) {
			return true;
		}
		if (inClanWars()) {
			return true;
	...
	public boolean rigUnlocked = false;
	public boolean augUnlocked = false;
	...
	public int getVespulaDamageCounter() {
		return raidsDamageCounters[5];
	}

	public void setVespulaDamageCounter(int damage) {
		this.raidsDamageCounters[5] = damage;
	}
	...
	public QuickSetsTab getQuickSets() {
		return handleSet;
	}
