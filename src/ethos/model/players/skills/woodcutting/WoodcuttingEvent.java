...
public class WoodcuttingEvent extends Event<Player> {
	...
	public void execute() {
		...
		xpBoostOsrs = (tree.getExperience() + tree.getExperience() / 10) * pieces;
		xpBoostReg =(tree.getExperience() * Config.WOODCUTTING_EXPERIENCE + tree.getExperience() * Config.WOODCUTTING_EXPERIENCE / 10) * pieces;
		osrsExperience = (tree.getExperience() + tree.getExperience() / 10) + xpBoostOsrs;
		experience = (tree.getExperience() * Config.WOODCUTTING_EXPERIENCE + tree.getExperience() * Config.WOODCUTTING_EXPERIENCE / 10) + xpBoostReg;
		if (canChop()) return;
	...
	private void handleDiary(Tree tree) {
		switch (tree) {
			...
			case TEAK:
				...
				break;
			case EVENT:
				if (Boundary.isIn(attachment, Boundary.HOME)) {
					if (Misc.random(50) == 10) {				
							attachment.getItems().addItemUnderAnyCircumstance(19835, 1);
							attachment.sendMessage("@blu@You have recieved a master clue scroll from the meat tree!");
					}
				break;
				}
			default:
	...
	private void handleRewards() {
		...
		if (Misc.random(tree.getPetChance()) / 20 == 10) {
		...
		if (Misc.random(tree.getPetChance()) == 2 && attachment.getItems().getItemCount(13322, false) == 0 && attachment.summonId != 13322) {
			PlayerHandler.executeGlobalMessage("[<col=CC0000>News</col>] @cr20@ <col=255>" + attachment.playerName + "</col> chopped down a nest and found the <col=CC0000>Beaver</col> pet!");
			DiscordMessager.sendPetsMessage("```[Pet Manager]" + attachment.playerName + " chopped down a nest and found the Beaver pet!```");
