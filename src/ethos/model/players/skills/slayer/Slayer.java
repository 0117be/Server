...
public class Slayer {
	...
	/**
	 * Represents NPC IDs of NPCs who can cause a Slayer Pet to spawn
	 */
	public static int[] PET_COUNTERPARTS = { 448, 481, 406, 414, 7272, 480, 421, 419, 435, 417, 446, 484, 7276, 437, 7277, 427, 3209, 6, 7279, 423, 411, 498, 1543, 4005, 415, 11, 7278 };
	...
	/**
	 * Determines if this player can ecounter Slayer Pets
	 */
	private boolean slayerPetsSpawn;
	/**
		...
		task.ifPresent(task -> {
			...
			if (name.equals(task.getPrimaryName()) || ArrayUtils.contains(task.getNames(), name)) {
				...
				master.ifPresent(m -> {
					...
					if (slayerPetsSpawn && Config.slayerPetSpawns) {
						//player.sendMessage("@red@slayer pet test");
						handleSlayerPetsSpawn(npc);
					}
					if (taskAmount == 0) {
						...
						this.points += m.getPointReward(0);
						 if (DoubleSlayerPoints.isDoubleSlayer()) {
								this.points += m.getPointReward(0);
								player.sendMessage("@red@You receive double points from your Slayer tasks today!");
						 }
						if (player.getItems().getItemCount(30015, true) >= 1) {	
								this.points += 10;
								player.sendMessage("@red@You received 10 extra points for having the slayer perk!");
						 }
						...
						bonusPoints+= rights.contains(Right.EXTREME_DONATOR) ? 20 : 0 ;
						bonusPoints+= rights.contains(Right.SUPER_DONATOR) ? 15 : 0 ;
						bonusPoints+= rights.contains(Right.DONATOR) ? 10 : 0 ;
						bonusPoints+= rights.contains(Right.SUPPORTER) ? 5 : 0 ;
			...
			if (chance == 0) {
				...
				switch(task.getPrimaryName()){
					...
					case "aberrant spectre":
						Server.npcHandler.spawnNpc(player, 7402, player.getX(), player.getY(), player.getHeight(), 1, 250, 31, 300, 80, true, false);
						superiorSpawned = true;
						player.sendMessage("@red@A superior foe has appeared...");
						break;
					case "deviant spectre":
						Server.npcHandler.spawnNpc(player, 7403, player.getX(), player.getY(), player.getHeight(), 1, 390, 31, 380, 220, true, false);
						superiorSpawned = true;
						player.sendMessage("@red@A superior foe has appeared...");
						break;
					case "dust devil":
	...
	public void handleSlayerPetsSpawn(NPC npc) {
		task.ifPresent(task -> {
			int chance = Misc.random(249);
			...
			if (chance == 35) {
				...
				switch(task.getPrimaryName()){
					case "crawling hand":
						if (player.getItems().getItemCount(4133, false) == 0 && player.summonId != 449) {
					PlayerHandler.executeGlobalMessage("[<col=CC0000>News</col>] @cr20@ <col=255>" + player.playerName
							+ " received a Crawling Hand Slayer pet! Congratulations!");
					DiscordMessager.sendPetsMessage("```[Pet Manager]" + player.playerName + " received a Crawling Hand Slayer pet! Congratulations!```");
					player.getItems().addItemUnderAnyCircumstance(4133, 1);
				}
					break;
					case "cave bug":
						if (player.getItems().getItemCount(4521, false) == 0 && player.summonId != 482) {
					PlayerHandler.executeGlobalMessage("[<col=CC0000>News</col>] @cr20@ <col=255>" + player.playerName
							+ " received a Cave Bug Slayer pet! Congratulations!");
					DiscordMessager.sendPetsMessage("```[Pet Manager]" + player.playerName + " received a Cave Bug Slayer pet! Congratulations!```");
					player.getItems().addItemUnderAnyCircumstance(4521, 1);
				}
					break;
					case "cave crawler":
						if (player.getItems().getItemCount(4134, false) == 0 && player.summonId != 407) {
					PlayerHandler.executeGlobalMessage("[<col=CC0000>News</col>] @cr20@ <col=255>" + player.playerName
							+ " received a Cave Crawler Slayer pet! Congratulations!");
					DiscordMessager.sendPetsMessage("```[Pet Manager]" + player.playerName + " received a Cave Crawler Slayer pet! Congratulations!```");
					player.getItems().addItemUnderAnyCircumstance(4134, 1);
				}
					break;
					case "banshee":
						if (player.getItems().getItemCount(4135, false) == 0 && player.summonId != 3984) {
					PlayerHandler.executeGlobalMessage("[<col=CC0000>News</col>] @cr20@ <col=255>" + player.playerName
							+ " received a Banshee Slayer pet! Congratulations!");
					DiscordMessager.sendPetsMessage("```[Pet Manager]" + player.playerName + " received a Banshee Slayer pet! Congratulations!```");
					player.getItems().addItemUnderAnyCircumstance(4135, 1);
				}
					break;
					case "twisted banshee":
						if (player.getItems().getItemCount(4135, false) == 0 && player.summonId != 3984) 
					...
					break;
					case "cave slime":
						if (player.getItems().getItemCount(4520, false) == 0 && player.summonId != 3985) {
					PlayerHandler.executeGlobalMessage("[<col=CC0000>News</col>] @cr20@ <col=255>" + player.playerName
							+ " received a Cave Slime Slayer pet! Congratulations!");
					DiscordMessager.sendPetsMessage("```[Pet Manager]" + player.playerName + " received a Cave Slime Slayer pet! Congratulations!```");
					player.getItems().addItemUnderAnyCircumstance(4520, 1);
				}
					break;
					case "rockslug":
						if (player.getItems().getItemCount(4136, false) == 0 && player.summonId != 422) {
					PlayerHandler.executeGlobalMessage("[<col=CC0000>News</col>] @cr20@ <col=255>" + player.playerName
							+ " received a Banshee Slayer pet! Congratulations!");
					DiscordMessager.sendPetsMessage("```[Pet Manager]" + player.playerName + " received a Banshee Slayer pet! Congratulations!```");
					player.getItems().addItemUnderAnyCircumstance(4136, 1);
				}
					break;
					case "cockatrice":
						if (player.getItems().getItemCount(4137, false) == 0 && player.summonId != 420) {
					PlayerHandler.executeGlobalMessage("[<col=CC0000>News</col>] @cr20@ <col=255>" + player.playerName
							+ " received a Cockatrice Slayer pet! Congratulations!");
					DiscordMessager.sendPetsMessage("```[Pet Manager]" + player.playerName + " received a Cockatrice Slayer pet! Congratulations!```");
					player.getItems().addItemUnderAnyCircumstance(4137, 1);
				}
					break;
					case "pyrefiend":
						if (player.getItems().getItemCount(4138, false) == 0 && player.summonId != 434) {
					PlayerHandler.executeGlobalMessage("[<col=CC0000>News</col>] @cr20@ <col=255>" + player.playerName
							+ " received a Pyrefiend Slayer pet! Congratulations!");
					DiscordMessager.sendPetsMessage("```[Pet Manager]" + player.playerName + " received a Pyrefiend Slayer pet! Congratulations!```");
					player.getItems().addItemUnderAnyCircumstance(4138, 1);
				}
					break;
					case "basilisk":
						if (player.getItems().getItemCount(4139, false) == 0 && player.summonId != 418) {
					PlayerHandler.executeGlobalMessage("[<col=CC0000>News</col>] @cr20@ <col=255>" + player.playerName
							+ " received a Basilisk Slayer pet! Congratulations!");
					DiscordMessager.sendPetsMessage("```[Pet Manager]" + player.playerName + " received a Basilisk Slayer pet! Congratulations!```");
					player.getItems().addItemUnderAnyCircumstance(4139, 1);
				}
					break;
					case "infernal mage":
						if (player.getItems().getItemCount(4140, false) == 0 && player.summonId != 443) {
					PlayerHandler.executeGlobalMessage("[<col=CC0000>News</col>] @cr20@ <col=255>" + player.playerName
							+ " received a Infernal Mage Slayer pet! Congratulations!");
					DiscordMessager.sendPetsMessage("```[Pet Manager]" + player.playerName + " received a Infernal Mage Slayer pet! Congratulations!```");
					player.getItems().addItemUnderAnyCircumstance(4140, 1);
				}
					break;
					case "bloodveld":
						if (player.getItems().getItemCount(4141, false) == 0 && player.summonId != 486) {
					PlayerHandler.executeGlobalMessage("[<col=CC0000>News</col>] @cr20@ <col=255>" + player.playerName
							+ " received a Bloodveld Slayer pet! Congratulations!");
					DiscordMessager.sendPetsMessage("```[Pet Manager]" + player.playerName + " received a Bloodveld Slayer pet! Congratulations!```");
					player.getItems().addItemUnderAnyCircumstance(4141, 1);
				}
					break;
					case "mutated bloodveld": //Add slayer task
						if (player.getItems().getItemCount(4141, false) == 0 && player.summonId != 486) {
					...
					break;
					case "jelly":
						if (player.getItems().getItemCount(4142, false) == 0 && player.summonId != 440) {
					PlayerHandler.executeGlobalMessage("[<col=CC0000>News</col>] @cr20@ <col=255>" + player.playerName
							+ " received a Jelly Slayer pet! Congratulations!");
					DiscordMessager.sendPetsMessage("```[Pet Manager]" + player.playerName + " received a Jelly Slayer pet! Congratulations!```");
					player.getItems().addItemUnderAnyCircumstance(4142, 1);
						}
					break;
					case "warped jelly": //Add slayer task
						if (player.getItems().getItemCount(4142, false) == 0 && player.summonId != 440) {
					...
					break;
					case "turoth":
						if (player.getItems().getItemCount(4143, false) == 0 && player.summonId != 426) {
					PlayerHandler.executeGlobalMessage("[<col=CC0000>News</col>] @cr20@ <col=255>" + player.playerName
							+ " received a Turoth Slayer pet! Congratulations!");
					DiscordMessager.sendPetsMessage("```[Pet Manager]" + player.playerName + " received a Turoth Slayer pet! Congratulations!```");
					player.getItems().addItemUnderAnyCircumstance(4143, 1);
						}
					break;
					case "cave horror":
						if (player.getItems().getItemCount(8900, false) == 0 && player.summonId != 3213) {
					PlayerHandler.executeGlobalMessage("[<col=CC0000>News</col>] @cr20@ <col=255>" + player.playerName
							+ " received a Cave Horror Slayer pet! Congratulations!");
					DiscordMessager.sendPetsMessage("```[Pet Manager]" + player.playerName + " received a Cave Horror Slayer pet! Congratulations!```");
					player.getItems().addItemUnderAnyCircumstance(8900, 1);
						}
					break;
					case "aberrant spectre":
						if (player.getItems().getItemCount(4144, false) == 0 && player.summonId != 3) {
					PlayerHandler.executeGlobalMessage("[<col=CC0000>News</col>] @cr20@ <col=255>" + player.playerName
							+ " received a Abberrant Spectre Slayer pet! Congratulations!");
					DiscordMessager.sendPetsMessage("```[Pet Manager]" + player.playerName + " received a Abberrant Spectre Slayer pet! Congratulations!```");
					player.getItems().addItemUnderAnyCircumstance(4144, 1);
						}		
						break;	
					case "deviant spectre":
						if (player.getItems().getItemCount(4144, false) == 0 && player.summonId != 3) {
					...
					player.getItems().addItemUnderAnyCircumstance(4144, 1);
						...
						break;	
					case "dust devil":
						if (player.getItems().getItemCount(4145, false) == 0 && player.summonId != 7249) {
					PlayerHandler.executeGlobalMessage("[<col=CC0000>News</col>] @cr20@ <col=255>" + player.playerName
							+ " received a Dust Devil Slayer pet! Congratulations!");
					DiscordMessager.sendPetsMessage("```[Pet Manager]" + player.playerName + " received a Dust Devil Slayer pet! Congratulations!```");
					player.getItems().addItemUnderAnyCircumstance(4145, 1);
						}			
						break;
					case "kurask":
						if (player.getItems().getItemCount(4146, false) == 0 && player.summonId != 410) {
					PlayerHandler.executeGlobalMessage("[<col=CC0000>News</col>] @cr20@ <col=255>" + player.playerName
							+ " received a Kurask Slayer pet! Congratulations!");
					DiscordMessager.sendPetsMessage("```[Pet Manager]" + player.playerName + " received a Kurask Slayer pet! Congratulations!```");
					player.getItems().addItemUnderAnyCircumstance(4146, 1);
						}			
					break;
					case "smoke devil":
						if (player.getItems().getItemCount(4145, false) == 0 && player.summonId != 7249) {
					...
					break;
					case "gargoyle":
						if (player.getItems().getItemCount(4147, false) == 0 && player.summonId != 412) {
					PlayerHandler.executeGlobalMessage("[<col=CC0000>News</col>] @cr20@ <col=255>" + player.playerName
							+ " received a Gargoyle Slayer pet! Congratulations!");
					DiscordMessager.sendPetsMessage("```[Pet Manager]" + player.playerName + " received a Gargoyle Slayer pet! Congratulations!```");
					player.getItems().addItemUnderAnyCircumstance(4147, 1);
						}	
					break;
					case "dark beast":
						if (player.getItems().getItemCount(6637, false) == 0 && player.summonId != 7520) {
					PlayerHandler.executeGlobalMessage("[<col=CC0000>News</col>] @cr20@ <col=255>" + player.playerName
							+ " received a Dark Beast Slayer pet! Congratulations!");
					DiscordMessager.sendPetsMessage("```[Pet Manager]" + player.playerName + " received a Dark Beast Slayer pet! Congratulations!```");
					player.getItems().addItemUnderAnyCircumstance(6637, 1);
						}
					break;
					case "abyssal demon":
						if (player.getItems().getItemCount(4149, false) == 0 && player.summonId != 416) {
					PlayerHandler.executeGlobalMessage("[<col=CC0000>News</col>] @cr20@ <col=255>" + player.playerName
							+ " received a Abyssal Demon Slayer pet! Congratulations!");
					DiscordMessager.sendPetsMessage("```[Pet Manager]" + player.playerName + " received a Abyssal Demon Slayer pet! Congratulations!```");
					player.getItems().addItemUnderAnyCircumstance(4149, 1);
						}
						break;
					case "nechryael":
						if (player.getItems().getItemCount(4148, false) == 0 && player.summonId != 8) {
					PlayerHandler.executeGlobalMessage("[<col=CC0000>News</col>] @cr20@ <col=255>" + player.playerName
							+ " received a Nechryael Slayer pet! Congratulations!");
					DiscordMessager.sendPetsMessage("```[Pet Manager]" + player.playerName + " received a Nechryael Slayer pet! Congratulations!```");
					player.getItems().addItemUnderAnyCircumstance(4148, 1);
						}	
					break;
				}
			}
	...
	@SuppressWarnings("unused")
	public boolean isPetCounter() {

		for (int PET_COUNTERPART : PET_COUNTERPARTS) {
			return true;
		}
		return false;
	}
	...
	public void cancelTask() {
		int rankPoints = 
		player.getRights().getPrimary() == Right.SUPPORTER ? 25 : 
		player.getRights().getPrimary() == Right.DONATOR ? 20 : 
		player.getRights().getPrimary() == Right.SUPER_DONATOR ? 15 : 
		player.getRights().getPrimary() == Right.EXTREME_DONATOR ? 10 : 
		player.getRights().getPrimary() == Right.LEGENDARY ? 7 : 30;
	...
	public void removeTask() {
		int rankPoints = 
				player.getRights().getPrimary() == Right.SUPPORTER ? 90 : 
					player.getRights().getPrimary() == Right.DONATOR ? 80 : 
					player.getRights().getPrimary() == Right.SUPER_DONATOR ? 70 : 
				player.getRights().getPrimary() == Right.EXTREME_DONATOR ? 50
				: player.getRights().getPrimary() == Right.LEGENDARY ? 30 : 100;
		if (!task.isPresent()) {
...
//			updatePoints();
		...
		case 160056:
			...
			if (player.getItems().playerHasItem(8901)) {
				slayerHelmet = 8901;
				slayerHelmetI = 11784;
			}
		...
		case 162042:
			...
			if (points < 275) {
				player.sendMessage("You need 275 slayer points to learn this.");
				return true;
			}
			points -= 275;
		...
		case 162043:
			...
			if (points < 300) {
				player.sendMessage("You need 300 slayer points to learn this.");
				return true;
			}
			points -= 300;
		...
		case 162044:
			...
			if (points < 100) {
				player.sendMessage("You need 100 slayer points to learn this.");
				return true;
			}
			points -= 100;
			slayerPetsSpawn = true;
			player.buySlayerTimer = System.currentTimeMillis();
			player.sendMessage("@red@ You've successfully learned to ecounter Slayer pets while on task");
			updatePoints();
			return true;
		...
		case 160058:
			...
			int Salve = 4081;
			int SalveI = 12017;
			if (!player.getItems().playerHasItem(4081) && !player.getItems().playerHasItem(10588)) {
				player.sendMessage("You need a Salve Amulet in your inventory to do this.");
				return true;
			}
			if (points < 350) {
				player.sendMessage("You need 350 slayer points to learn this.");
			...
			if (player.getItems().playerHasItem(4081)) {
				Salve = 4081;
				SalveI = 12017;
			}
			if (player.getItems().playerHasItem(10588)) {
				Salve = 10588;
				SalveI = 12018;
			}
			points -= 350;
			player.getItems().deleteItem(Salve, 1);
			player.getItems().addItem(SalveI, 1);
			player.buySlayerTimer = System.currentTimeMillis();
			player.sendMessage("@red@You create an Imbued Salve Amulet.@red@");
			return true;
	...
	private enum SLAYER_HELMETS {
		...
		TURQOUISE(21888, 21890, 21907),
		HYDRA(23073, 23075, 23077),
		TZKAL(25912, 25912, 25912),
	...
	public boolean isSlayerPetSpawns() {
		return slayerPetsSpawn;
	}
	public void setSlayerPetSpawns(boolean slayerPetsSpawn) {
		this.slayerPetsSpawn = slayerPetsSpawn;
	}
