...
public class Cooking extends SkillHandler {
	...
	public static int[] fishIds = { 315, 2309, 319, 325, 347, 355, 333, 339, 351, 329, 361, 365, 2140, 2142, 379, 373,
			385, 397, 391, 7946, 11936, 3144, 13441, 20855, 20857, 20859, 20861, 20863, 20865, 20867 };
	...
	public static void cookThisFood(Player p, int i, int object) {
		...
		case 3142:
			...
			break;
			//start of raids cooking
		case 20855:
			cookFish(p, i, 10, 1, 20856, 20856, object);
			break;
		case 20857:
			cookFish(p, i, 13, 15, 20858, 20858, object);
			break;
		case 20859:
			cookFish(p, i, 16, 30, 20860, 20860, object);
			break;
		case 20861:
			cookFish(p, i, 19, 45, 20862, 20862, object);
			break;
		case 20863:
			cookFish(p, i, 22, 60, 20864, 20864, object);
			break;
		case 20865:
			cookFish(p, i, 25, 75, 20866, 20866, object);
			break;
		case 20867:
			cookFish(p, i, 28, 90, 20868, 20868, object);
			break;
			//end of raids cooking
		default:
		...
		Server.getEventHandler().submit(new Event<Player>("skilling", c, 2) {
			...
			public void execute() {
				...
				if (attachment.playerSkillProp[7][6] >= fishStopsBurning(attachment.playerSkillProp[7][0]) || cookFish(c)) {
					...
					if (Misc.random(65) == 1) {	
						c.getItems().addItemToBank(20791, 1);
						c.sendMessage("@blu@A skilling supply crate has been added to your bank!");
					}
