...
public class Runecrafting extends SkillHandler {
	...
	private enum RUNECRAFTING_DATA {
		...
		BLOOD(25380, 565, 15, new int[] { 77, 89, 96 }, 18800, 20691), 
		WRATH(32492, 21880, 17, new int[] { 95 }, 17450, 21990), 
		SOUL(25377, 566, 16, new int[] { 90, 95, 125 }, 18000, 20687);
	...
	public static void execute(Player player, int objectId) {
		...
		/*		if (name.equals("nature")) {
				...
				Achievements.increase(player, AchievementType.RUNECRAFTING, multiply);
				...
				player.getPA().requestUpdates();
					...
					if (Misc.random(data.getPetChance()) == 18) {
						...
						DiscordMessager.sendPetsMessage("```[Pet Manager]" + player.playerName + " successfully crafted a Rift guardian pet!```");
