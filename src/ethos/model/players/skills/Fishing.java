...
public class Fishing extends SkillHandler {
	...
	public static int[] isHarpoon = { 311, 21028, 21031 };
	/**
	 * Fishing data
	...
	private static void clueBottles(Player player) {
		int chance = player.playerSkillProp[10][10] / 70;
		...
		if (Misc.random(chance) == 1) {
			...
			if (bottleRoll < 6) {
				...
				player.getItems().addItemToBank(13649, 1);
			} else if (bottleRoll > 5 && bottleRoll < 9) {
				player.getItems().addItemToBank(19841, 1);
			} else {
				player.getItems().addItemToBank(13650, 1);
			}
		}
					...
					public void execute(CycleEventContainer container) {
							...
							if (Misc.random(12000) == 5555) {
							 ...
							 if (Misc.random(player.playerSkillProp[10][10]) == 2 && player.getItems().getItemCount(13320, true) == 0 && player.summonId != 13320) {
								 PlayerHandler.executeGlobalMessage("[<col=CC0000>News</col>] @cr20@ <col=255>" + player.playerName + "</col> caught a fish and a <col=CC0000>Heron</col> pet!");
									DiscordMessager.sendPetsMessage("```[Pet Manager]" + player.playerName + " caught a fish and a Heron pet!```");
						...
						case 371:
							if (Boundary.isIn(player, Boundary.CATHERBY_BOUNDARY)) {
								...
								Achievements.increase(player, AchievementType.FISH_SWORDFISH, 1);
							}
							else 
								Achievements.increase(player, AchievementType.FISH_SWORDFISH, 1);
							break;
						case 383:
								Achievements.increase(player, AchievementType.FISH_SHARK, 1);
	...
	private static boolean hasFishingEquipment(Player c, int equipment) {
		if (!c.getItems().playerHasItem(equipment)) {
			if (equipment == 311) {
				if (!c.getItems().playerHasItem(311) && !c.getItems().playerHasItem(10129) && c.playerEquipment[3] != 10129 && !c.getItems().playerHasItem(21028) && c.playerEquipment[3] != 21028 && !c.getItems().playerHasItem(21031) && c.playerEquipment[3] != 21031) {
