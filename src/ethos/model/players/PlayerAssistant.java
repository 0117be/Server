...
public class PlayerAssistant {
	...
	public void assembleSlayerHelmetI() {
		if (!c.getSlayer().isHelmetCreatable()) {
			c.sendMessage("You don't have the knowledge to do this. You must learn how to first.");
			return;
		}
	...
	if (c.getItems().playerHasItem(4166) && c.getItems().playerHasItem(4168) && c.getItems().playerHasItem(4164) && c.getItems().playerHasItem(4551)
			&& c.getItems().playerHasItem(11784)) {
		c.sendMessage("@blu@You assemble the pieces and create a full slayer helmet!");
		c.getItems().deleteItem(4166, 1);
		c.getItems().deleteItem(4164, 1);
		c.getItems().deleteItem(4168, 1);
		c.getItems().deleteItem(4551, 1);
		c.getItems().deleteItem(11784, 1);
	...
	public void sendItemToSlotWithOpacity(int frame, int item, int slot, int amount, boolean opaque) {
		...
		c.outStream.createFrameVarSizeWord(34);
		c.outStream.writeDWord(frame);
		c.outStream.writeByte(slot);
		c.outStream.writeDWord(bitpackedValue);
		c.outStream.writeByte(255);
		c.outStream.writeDWord(amount);
		c.outStream.endFrameVarSizeWord();
	}
	...
	public void sendStringContainer(int containerInterfaceId, String...strings) {
		if (c.outStream != null) {
			...
			c.outStream.writeDWord(containerInterfaceId);
			c.outStream.writeDWord(strings.length);
	...
	public void sendInterfaceSet(int mainScreenInterface, int inventoryInterface) {
		 ...
		 c.getOutStream().writeDWord(inventoryInterface);
	...
	public void alchemy(int itemId, String alch) {
		...
		case "high":
			if (System.currentTimeMillis() - c.alchDelay > 2000) {
				...
				for (int[] items : EquipmentSet.HARDCORE_ARMOUR.getEquipment()) {
					if (Misc.linearSearch(items, itemId) > -1) {
			...
			if (spell == aBoltData[0]) {
				if (bolt == aBoltData[1]) {
					...
					case 6003:
						...
						break;
					case 23649:
						if (!c.getCombat().checkMagicReqs(93)) {
							return;
						}
						break;
					}
	...
	public void applyDead() {
		...
		if (Objects.isNull(duelSession)) {
			...
			if (killer != null) {
				...
				if (killer instanceof Player) {
					...
					if (c.killerId != c.getIndex()) {
						if (c.inWild()) {
							...
							if (c.getMode().getType().equals(ModeType.HARDCORE) && (!Boundary.isIn(c, Boundary.HARDCORESAFEZONE))) {
								c.setMode(Mode.forType(ModeType.IRON_MAN));
								c.getRights().remove(Right.HARDCORE);
								c.getRights().setPrimary(Right.IRONMAN);
								DiscordMessager.sendAnnouncement("```[RIP] Hardcore Ironman " + Misc.capitalize(c.playerName) + "has made some mistakes!</col>```");
								PlayerHandler.executeGlobalMessage("<img=23></img>[@red@RIP] Hardcore Ironman " + Misc.capitalize(c.playerName) + " has made some mistakes!</col>");
							//	PlayerHandler.executeGlobalMessage("<img=25></img>[<col=4324>News</col>] <col=23442> Hardcore Ironman " + Misc.capitalize(c.playerName) + "</col> has died! <col=23442>");
								c.sendMessage("Your rank has been reverted to Ironman");
							}
							...
							if (!playerKiller.getPlayerKills().killedRecently(c.connectedFrom)
								...
								if (Boundary.isIn(c, Boundary.SAFEPK)) {
									...
									int bonuspkp = 0;
									bonuspkp+= rights.contains(Right.LEGENDARY) ? 5 : 0 ;
									bonuspkp+= rights.contains(Right.EXTREME_DONATOR) ? 4 : 0 ;
									bonuspkp+= rights.contains(Right.SUPER_DONATOR) ? 3 : 0 ;
									bonuspkp+= rights.contains(Right.DONATOR) ? 2 : 0 ;
									bonuspkp+= rights.contains(Right.SUPPORTER) ? 1 : 0 ;
				...
				if (c.getMode().getType().equals(ModeType.HARDCORE) && Boundary.isIn(c, Boundary.HARDCORESAFEZONE)) {
					...
					c.setMode(Mode.forType(ModeType.HARDCORE));
					c.getRights().setPrimary(Right.HARDCORE);
					c.sendMessage("@red@You have died in a Safezone and do not lose your Hardcore status");
				}
				if	(c.getRights().isOrInherits(Right.HELPER)) {
						c.setMode(Mode.forType(ModeType.HARDCORE));
						c.sendMessage("@red@You have died in a Safezone and do not lose your Hardcore status");
				...
				if (c.getMode().getType().equals(ModeType.HARDCORE) && !Boundary.isIn(c, Boundary.HARDCORESAFEZONE)) {
					...
					DiscordMessager.sendAnnouncement("```[RIP] Hardcore Ironman " + Misc.capitalize(c.playerName) + " has made some mistakes!</col>");
					PlayerHandler.executeGlobalMessage("<img=23></img>[@red@RIP] Hardcore Ironman " + Misc.capitalize(c.playerName) + " has made some mistakes!</col>```");
					//PlayerHandler.executeGlobalMessage("<img=23></img>[<col=43247>News</col>]<col=46542> Hardcore " + Misc.capitalize(c.playerName) + "</col> has died! <col=43247>");
					c.sendMessage("Your rank has been reverted to Ironman");
				}
		...
		if (duelSession != null && duelSession.getStage().getStage() == MultiplayerSessionStage.FURTHER_INTERATION) {
			if (!duelSession.getWinner().isPresent()) {
				...
				Achievements.increase(c, AchievementType.DUEL_WINNER, 1);
	...
	public void giveLife() {
		...
		if (!c.inDuelArena() && !Boundary.isIn(c, Boundary.DUEL_ARENA) && Boundary.isIn(c, Boundary.WILDERNESS)&& !VorkathInstance.inVorkath(c)
			...
			//CHANGE THIS BECAUSE NOW OSRS MODE LOSES ALL ITEMS ON DEATH
	...
	} else if (Boundary.isIn(c, Boundary.TOB)) {
		c.getPA().movePlayer(c.getTob().getStartLocation().getX(),c.getRaids().getStartLocation().getY(),c.getTob().getHeight(c.getTob().tobLeader));
		c.getTob().currentRoom=0;
	}
		...
		//	c.getZulrahLostItems().store();
			c.talkingNpc = 2040;
			c.getDH().sendNpcChat("It looks like Zulrah beat you.", "better luck next time!");
		} else if (Boundary.isIn(c, Boundary.CERBERUS_BOSSROOMS)) {
			c.getPA().movePlayer(1309, 1250, 0);
			//c.getCerberusLostItems().store();
			c.talkingNpc = 5870;
			c.getDH().sendNpcChat("It looks like Cerberus beat you.", "Better luck next time!");
		} else if (Boundary.isIn(c, Boundary.SKOTIZO_BOSSROOM)) {
			c.getPA().movePlayer(1665, 10045, 0);
		//	c.getSkotizoLostItems().store();
			c.talkingNpc = 7283;
			c.getDH().sendNpcChat("It looks like Skotizo beat you.", "better luck next time!");
			...
			c.getPA().movePlayer(c.getTob().getStartLocation().getX(),c.getTob().getStartLocation().getY(),c.getTob().getHeight(c.getTob().tobLeader));
		} else if (Boundary.isIn(c, Boundary.RAIDS)) {
			c.getPA().movePlayer(c.getRaids().getStartLocation().getX(),c.getRaids().getStartLocation().getY(),c.getRaids().getHeight(c.getRaids().raidLeader));
		} else if (Boundary.isIn(c, Boundary.OLM)) {
			c.getPA().movePlayer(c.getRaids().getStartLocation().getX(),c.getRaids().getStartLocation().getY(),c.getRaids().getHeight(c.getRaids().raidLeader));
	...
	private void updateLife() {
		...
		if (c.hasFollower) {
			if (c.summonId > 0) {
				Pets pet = PetHandler.forItem(c.summonId);
				if (pet != null) {
	...
	public void startLeverTeleport(int x, int y, int height) {
		...
		if (c.inClanWars() || c.inClanWarsSafe()) {
			c.sendMessage("@cr10@You can not teleport from here, speak to the PK_District to leave.");
			return;
		}
	...
	public boolean canTeleport(String type) {
		...
		if (c.inClanWars() || c.inClanWarsSafe()) {
			c.sendMessage("@cr10@You can not teleport from here, speak to the PK District Manager to leave.");
			return false;
		}
		if (System.currentTimeMillis() - c.lastSpear < 3000) {
	...
	public void startTeleport(int x, int y, int height, String teleportType, boolean homeTeleport) {
		...
		if (!c.isDead && c.teleTimer == 0 && c.respawnTimer == -6) {
			...
			if (teleportType.equalsIgnoreCase("modern") || teleportType.equals("glory")) {
				...
				c.teleTimer = 13;
				c.teleEndAnimation = 65535;
			} else if (teleportType.equals("seedpod")) {
	...
	public void followNpc() {
		...
		boolean projectile = c.usingOtherRangeWeapons || c.usingBow || c.usingMagic || c.autocasting
				|| c.getCombat().usingCrystalBow() || c.getCombat().usingCrawsBow();
		if (!projectile
	...
	public void followPlayer() {
		...
		boolean projectile = c.usingOtherRangeWeapons || c.usingBow || c.usingMagic || c.getCombat().usingCrystalBow() || c.getCombat().usingCrawsBow();
		if (!projectile
				|| projectile && (PathChecker.isProjectilePathClear(c.absX, c.absY, c.heightLevel, otherX, otherY)
						|| PathChecker.isProjectilePathClear(otherX, otherY, c.heightLevel, c.absX, c.absY))) {
	...
	public void levelUp(int skill) {
		...
		if (c.maxRequirements(c) && !c.getRights().isOrInherits(Right.OWNER) && !c.getRights().isOrInherits(Right.ADMINISTRATOR) && !c.getRights().isOrInherits(Right.MANAGER)) {
			PlayerHandler.executeGlobalMessage("<shad=7000>@gre@[Skilling]@cr20@ " + Misc.capitalize(c.playerName) + " has reached max total level!");
			DiscordMessager.sendAnnouncement("```" + Misc.capitalize(c.playerName) + " has reached max total level!```");
		}
		if (getLevelForXP(c.playerXP[skill]) == 99 && !c.getRights().isOrInherits(Right.OWNER) && !c.getRights().isOrInherits(Right.ADMINISTRATOR)) {
			Skill s = Skill.forId(skill);
			PlayerHandler.executeGlobalMessage(
					"<shad=7000>@gre@[Skilling]@cr20@ " + Misc.capitalize(c.playerName) + " has reached level 99 " + s.toString() + "!");
			DiscordMessager.sendAnnouncement(
					"```" + Misc.capitalize(c.playerName) + " has reached level 99 " + s.toString() + "!```");
		}
		c.dialogueAction = 0;
		c.nextChat = 0;
	}
	...
	public boolean addSkillXP(int amount, int skill, boolean dropExperience) {
		...
		int oldExperience = c.playerXP[skill];
			if (oldExperience < 100_000_000 && oldExperience + amount >= 100_000_000) {
		Skill s = Skill.forId(skill);
		PlayerHandler.executeGlobalMessage("<shad=7000>@gre@[Skilling]@cr20@ " + Misc.capitalize(c.playerName) + " has reached 100M XP in "
				+ s.toString() + ", congratulations.");
		DiscordMessager.sendAnnouncement("```[Grind Report]" + c.playerName + " has reached 100m experience in " + s.toString() +" Congratulations!```");

			}
			if (oldExperience < 150_000_000 && oldExperience + amount >= 150_000_000) {
		Skill s = Skill.forId(skill);
		PlayerHandler.executeGlobalMessage("<shad=7000>@gre@[Skilling]@cr20@ " + Misc.capitalize(c.playerName) + " has reached 150M XP in "
				+ s.toString() + ", congratulations.");
		DiscordMessager.sendAnnouncement("```[Grind Report]" + c.playerName + " has reached 150m experience in " + s.toString() +" Congratulations!```");

			}
	if (oldExperience < 200_000_000 && oldExperience + amount >= 200_000_000) {
		Skill s = Skill.forId(skill);
		PlayerHandler.executeGlobalMessage("<shad=7000>@gre@[Skilling]@cr20@ " + Misc.capitalize(c.playerName) + " has reached 200M XP in "
				+ s.toString() + ", congratulations.");
		DiscordMessager.sendAnnouncement("```[Grind Report]" + c.playerName + " has reached 200m experience in " + s.toString() +" Congratulations!```");
	}
	if (c.getItems().getItemCount(30009, true) == 1 && skill != Skill.DEFENCE.getId() && skill != Skill.ATTACK.getId()&& skill != Skill.STRENGTH.getId() && skill != Skill.MAGIC.getId() && skill != Skill.RANGED.getId() && skill != Skill.HITPOINTS.getId()) {	
		
			amount *= 1.20;											
		}
	if (c.getItems().getItemCount(30010, true) == 1 && skill == Skill.DEFENCE.getId() && skill == Skill.ATTACK.getId()&& skill == Skill.STRENGTH.getId() && skill == Skill.MAGIC.getId() && skill == Skill.RANGED.getId() && skill == Skill.HITPOINTS.getId()) {	
		
		amount *= 1.20;											
	}
	if (c.getRights().isOrInherits(Right.OSRS) && !DoubleExperience.isDoubleExperience(c) && !Config.BONUS_XP_WOGW && c.bonusXpTime >= 1) {
		amount *= 2.0;
		// If within thedonator zone, VIP accounts get bonus xp while bonus weekend is
		// on
	} else if (c.getRights().isOrInherits(Right.OSRS) && Boundary.isIn(c, Boundary.DONATOR_ZONE) && DoubleExperience.isDoubleExperience(c) && !Config.BONUS_XP_WOGW && c.bonusXpTime >= 1) {
		amount *= 4.0;
	} else if (c.getRights().isOrInherits(Right.OSRS) && Boundary.isIn(c, Boundary.DONATOR_ZONE) && !DoubleExperience.isDoubleExperience(c) && !Config.BONUS_XP_WOGW && c.bonusXpTime >= 1) {
		amount *= 2.5;
	} else if (c.getRights().isOrInherits(Right.OSRS) && !Boundary.isIn(c, Boundary.DONATOR_ZONE) && !DoubleExperience.isDoubleExperience(c) && Config.BONUS_XP_WOGW && c.bonusXpTime >= 1) {
		amount *= 2.5;
	//Ruby Donator Zone
	} else if (c.getRights().isOrInherits(Right.OSRS) && Boundary.isIn(c, Boundary.EDONATOR_ZONE) && DoubleExperience.isDoubleExperience(c) && !Config.BONUS_XP_WOGW && c.bonusXpTime >= 1) {
		amount *= 4.5;
	} else if (c.getRights().isOrInherits(Right.OSRS) && Boundary.isIn(c, Boundary.EDONATOR_ZONE) && !DoubleExperience.isDoubleExperience(c) && !Config.BONUS_XP_WOGW && c.bonusXpTime >= 1) {
		amount *= 3.0;
	} else if (c.getRights().isOrInherits(Right.OSRS) && !Boundary.isIn(c, Boundary.EDONATOR_ZONE) && !DoubleExperience.isDoubleExperience(c) && Config.BONUS_XP_WOGW && c.bonusXpTime >= 1) {
	...
	//Diamond Donator Zone
		} else if (c.getRights().isOrInherits(Right.OSRS) && Boundary.isIn(c, Boundary.DIAMOND_ZONE) && DoubleExperience.isDoubleExperience(c) && !Config.BONUS_XP_WOGW && c.bonusXpTime >= 1) {
			amount *= 4.8;
		} else if (c.getRights().isOrInherits(Right.OSRS) && Boundary.isIn(c, Boundary.DIAMOND_ZONE) && !DoubleExperience.isDoubleExperience(c) && !Config.BONUS_XP_WOGW && c.bonusXpTime >= 1) {
			amount *= 3.2;
		} else if (c.getRights().isOrInherits(Right.OSRS) && !Boundary.isIn(c, Boundary.DIAMOND_ZONE) && !DoubleExperience.isDoubleExperience(c) && Config.BONUS_XP_WOGW && c.bonusXpTime >= 1) {
	 ...
	 /**
	  * Donator xp boosts without book XP boost
	  */
	if (Boundary.isIn(c, Boundary.DONATOR_ZONE) && !DoubleExperience.isDoubleExperience(c) && !Config.BONUS_XP_WOGW && c.bonusXpTime == 0) {
		amount *= 1.25;
		// If within thedonator zone, VIP accounts get bonus xp while bonus weekend is
		// on
	} else if (Boundary.isIn(c, Boundary.EDONATOR_ZONE) && !DoubleExperience.isDoubleExperience(c) && !Config.BONUS_XP_WOGW && c.bonusXpTime == 0) {
		amount *= 1.75;
	} else if (Boundary.isIn(c, Boundary.DIAMOND_ZONE) && !DoubleExperience.isDoubleExperience(c) && !Config.BONUS_XP_WOGW && c.bonusXpTime == 0) {
		amount *= 1.90;
	} else if (Boundary.isIn(c, Boundary.COMBAT_DUMMY)) {
		amount *= 0;
	} else if (Boundary.isIn(c, Boundary.DONATOR_ZONE) && DoubleExperience.isDoubleExperience(c) && !Config.BONUS_XP_WOGW && c.bonusXpTime == 0) {
		amount *= 1.75;
		//zone and well without weekend
	} else if (Boundary.isIn(c, Boundary.DONATOR_ZONE) && !DoubleExperience.isDoubleExperience(c) && Config.BONUS_XP_WOGW && c.bonusXpTime == 0) {
		amount *= 1.75;
		//zone, weekend, and well
	} else if (Boundary.isIn(c, Boundary.DONATOR_ZONE) && DoubleExperience.isDoubleExperience(c) && Config.BONUS_XP_WOGW && c.bonusXpTime == 0) {
		amount *= 2.15;
		//EDZ
	} else if (Boundary.isIn(c, Boundary.EDONATOR_ZONE) && DoubleExperience.isDoubleExperience(c) && !Config.BONUS_XP_WOGW && c.bonusXpTime == 0) {
		...
		amount *= 2.80;
		/**
		 * Donator xp boosts with book XP boost
		 */
	} else if (Boundary.isIn(c, Boundary.DONATOR_ZONE) && !DoubleExperience.isDoubleExperience(c) && !Config.BONUS_XP_WOGW && c.bonusXpTime > 0) {
		amount *= 1.60;
		// If within thedonator zone, VIP accounts get bonus xp while bonus weekend is
...
/**
* Bonus weekend multipliers
*/
} else if (DoubleExperience.isDoubleExperience(c) && c.bonusXpTime == 0) {
amount *= 1.50;
} else if (DoubleExperience.isDoubleExperience(c) && Config.BONUS_XP_WOGW && c.bonusXpTime == 0) {
amount *= 2.00;
	} else if (DoubleExperience.isDoubleExperience(c) && !Config.BONUS_XP_WOGW && c.bonusXpTime == 0) {
		amount *= 1.50;
	} else if (DoubleExperience.isDoubleExperience(c) && !Config.BONUS_XP_WOGW && c.bonusXpTime > 0) {
		...
		amount *= 2.15;
/**
* WOGW Bonus XP multipliers
*/
	} else if (Config.BONUS_XP_WOGW && DoubleExperience.isDoubleExperience(c) && c.bonusXpTime == 0) {
		amount *= 2.00;
		// Else (20%)
	} else if (Config.BONUS_XP_WOGW && !DoubleExperience.isDoubleExperience(c) && c.bonusXpTime == 0) {
		amount *= 1.50;
		/**
		 * Book bonus XP multipliers
		 */
	} else if (c.bonusXpTime > 0 && DoubleExperience.isDoubleExperience(c) && !Config.BONUS_XP_WOGW ) {
		amount *= 1.85;
		// Else (20%)
	} else if (c.bonusXpTime > 0 && DoubleExperience.isDoubleExperience(c) && Config.BONUS_XP_WOGW ) {
		amount *= 2.35;
	...
	public void updateQuestTab(){
		if (c.getDiaryManager().getVarrockDiary().hasDoneAll()) {
		...
		//sendFrame126("@or1@Varrock", 29480);
		if (c.getDiaryManager().getArdougneDiary().hasDoneAll()) {
	...
	//	sendFrame126("@or1@Ardougne", 29481);
		if (c.getDiaryManager().getDesertDiary().hasDoneAll()) {
		...
		//sendFrame126("@or1@Desert", 29482);
		if (c.getDiaryManager().getFaladorDiary().hasDoneAll()) {
		...
		//sendFrame126("@or1@Falador", 29483);
		if (c.getDiaryManager().getFremennikDiary().hasDoneAll()) {
			sendFrame126("@gre@Fremennik", 29484);

		} else {
			sendFrame126("@or1@Fremmennik", 29484);
	}
		//sendFrame126("@or1@Fremennik", 29484);
		if (c.getDiaryManager().getKandarinDiary().hasDoneAll()) {
		...
		//sendFrame126("@or1@Kandarin", 29485);
		if (c.getDiaryManager().getKaramjaDiary().hasDoneAll()) {
		...
		//sendFrame126("@or1@Karamja", 29486);
		if (c.getDiaryManager().getLumbridgeDraynorDiary().hasDoneAll()) {
			sendFrame126("@gre@Lumbrige & Draynor", 29487);

		} else {
			sendFrame126("@or1@Varrock", 29487);
	}
		//sendFrame126("@or1@Lumbridge & Draynor", 29487);
		if (c.getDiaryManager().getMorytaniaDiary().hasDoneAll()) {
		...
		//sendFrame126("@or1@Morytania", 29488);
		if (c.getDiaryManager().getWesternDiary().hasDoneAll()) {
	...
	public void handleGlory(int gloryId) {
		...
		c.getPA().spellTeleport(3087, 3506, 0, false);
	...
	public void useOperate(int itemId) {
		...
		case 10507:
			if (c.getItems().isWearingItem(10507)) {
				if (System.currentTimeMillis() - c.lastPerformedEmote < 60)
					return;
