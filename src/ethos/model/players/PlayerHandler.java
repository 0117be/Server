...
public class PlayerHandler {
	...
	public static Optional<Player> getOptionalPlayer(String name) {
		return getPlayers().stream().filter(Objects::nonNull).filter(client -> client.playerName.equalsIgnoreCase(name))
				.findFirst();
	}
	...
	/**
	 * Create an int array of the specified length, containing all values between 0
	 * and length once at random positions.
	 * 
				...
				try {
					...
					if (players[randomOrder[i]].disconnected
						...
						DuelSession duelSession = (DuelSession) Server.getMultiplayerSessionListener()
								.getMultiplayerSession(players[randomOrder[i]], MultiplayerSessionType.DUEL);
