...
public class ClickingButtons implements PacketType {
	...
	public void processPacket(final Player c, int packetType, int packetSize) {
		Special checkspec = Specials.forWeaponId(c.playerEquipment[c.playerWeapon]);
		...
		if (c.getTutorial().isActive()) {
			if(actionButtonId == 132014) {
				c.getPA().sendFrame126("Standard playing style. Enjoy playing Revelation with", 33815);
			...
			if(actionButtonId == 132017) {
				if(c.gamemode == 0) {
					...
					c.sendMessage("You have selected normal gamemode! Enjoy Revelation!");
				} else if(c.gamemode == 1) {
					c.sendMessage("You have selected Ironman gamemode! Enjoy Revelation!");
					c.setMode(Mode.forType(ModeType.IRON_MAN));
					c.getRights().setPrimary(Right.IRONMAN);
				} else {
					c.sendMessage("You have selected Ultimate Ironman gamemode! Enjoy Revelation!");
		...
		if(Teleports.isQuickSet(c, actionButtonId)) {
			c.getQuickSets().handleSet(c, actionButtonId);
		}
		...
		int[] spellIds = { 4128, 4130, 4132, 4134, 4136, 4139, 4142, 4145, 4148, 4151, 4153, 4157, 4159, 4161, 4164,
				4165, 4129, 4133, 4137, 6006, 6007, 6026, 6036, 6046, 6056, 4147, 6003, 23649, 47005, 4166, 4167, 4168, 48157,
		...
		case 253248: //cities btn
			...
			break;
		case 165162:
			c.getDH().sendDialogues(659, -1);
			return;
	case 165163:
		c.getDH().sendDialogues(660, -1);
		return;
	case 165183:
		c.getDH().sendDialogues(6600, -1);
		return;
	case 165185:
		c.getDH().sendDialogues(6601, -1);
		return;
		//Teleport Selections	
		case 254007:
			break;
		...
		case 132015:
			...
			c.getPA().itemOnInterface(4413, 1, 33820, 2);
			c.getPA().itemOnInterface(386, 50, 33820, 3);
			c.getPA().itemOnInterface(303, 1, 33820, 4);
			c.getPA().itemOnInterface(301, 1, 33820, 5);
			c.getPA().itemOnInterface(12810, 1, 33820, 6);

			c.getPA().itemOnInterface(12811, 0, 33821, 0);
			c.getPA().itemOnInterface(12812, 0, 33821, 1);
			c.getPA().itemOnInterface(-1, 0, 33821, 2);
			c.getPA().itemOnInterface(-1, 0, 33821, 3);
			c.getPA().itemOnInterface(-1, 0, 33821, 4);
			c.getPA().itemOnInterface(-1, 0, 33821, 5);
			...
			break;
		case 132016:
			c.getPA().sendFrame126("In addition to the standard Ironman rules, a Hardcore", 33815);
			c.getPA().sendFrame126("Ironman will lose their spot, on the HCIM", 33816);
			c.getPA().sendFrame126("highscores upon death.(7% droprate boost)", 33817);
			...
			c.getPA().itemOnInterface(303, 1, 33820, 0);
			c.getPA().itemOnInterface(20792, 1, 33820, 1);
			c.getPA().itemOnInterface(20794, 1, 33820, 2);
			c.getPA().itemOnInterface(20796, 0, 33820, 3);
			c.getPA().itemOnInterface(-1, 0, 33820, 4);
			c.getPA().itemOnInterface(-1, 0, 33820, 5);
			c.getPA().itemOnInterface(-1, 0, 33820, 6);
			...
			case 183156:
				switch(c.boxCurrentlyUsing) {
					case 13346: //ultra rare
						...
						break;
					case 13656: //ultra rare
						c.getXmasPresent().spin();
						break;
					case 6677: //ultra rare
						c.getMegaBox().spin();
						break;
				}
				break;
		...
		case 113238:
			if (Config.BONUS_XP_WOGW == true) {
				...
				c.sendMessage("@cr10@ <col=6666FF>Wogw is granting double drops for another "
		...
		case 40168:
			...
			if(c.questTab == 2) {
				c.forcedChat("I currently have " + c.getSlayer().getPoints() + " vote points.");
			}
			if(c.questTab == 3) {
				c.sendMessage("@gre@Collection Log is still a work in progress. Check back at the end of the week!");
		...
		case 40170:
			...
			if(c.questTab == 3) {
				c.getPA().sendFrame126("http://revelation-rsps.com/forums/index.php?/forum/12-guides/", 12000);
			}
			break;
		case 40171:
			if(c.questTab == 0) {
			...
			if(c.questTab == 2) {
				c.forcedChat("I currently have " + c.pcPoints + " pest control points.");
			}
			if(c.questTab == 3) {
...
//	c.getDH().sendDialogues(12000, -1);
				...
				int[] frames1 = { 8149, 8150, 8151, 8152, 8153, 8154, 8155, 8156, 8157, 8158, 8159, 8160, 8161, 8162, 8163, 8164, 8165, 8166, 8167, 8168, 8169, 8170, 8171, 8172, 8173,
				...
				for (Map.Entry<String, Integer> entry : c.getNpcDeathTracker().getTracker().entrySet()) {
					...
					if (frameIndex1 > frames1.length - 1) {
						break;
					}
					if (entry.getValue() > 0) {
						c.getPA().sendFrame126("@blu@" + WordUtils.capitalize(entry.getKey().toLowerCase()) + ": @red@" + entry.getValue(), frames1[frameIndex1]);
						frameIndex1++;
					}
				}
				c.getPA().showInterface(8134);
		...
		case 40173:
			if(c.questTab == 0) {
				...
				c.forcedChat("I have played for " + time);				
			} 
			if(c.questTab == 1) {
				c.getDiaryManager().getKandarinDiary().display();
			}
			if(c.questTab == 2) {
				c.forcedChat("I currently have " + c.dailyPoints + " daily task points.");
			}
			if(c.questTab == 3) {
				if(MonsterHunt.getCurrentLocation() != null) {
					c.sendMessage("@red@<shad=7000>[Event Boss] @bla@Current Location: " + MonsterHunt.getCurrentLocation().getLocationName());
					c.sendMessage("@red@<shad=7000>[Event Boss] @bla@Current Monster: " + MonsterHunt.getName());
				//	c.sendMessage("@red@[Wildy Pursuit] @bla@ Type ::telepursuit to get to the monsters location. @red@Wilderness!!");
				} else {
					c.sendMessage("@red@[Wildy Pursuit] @bla@No monster is currently in pursuit.");
				}
		...
		case 40174:
			...
			if(c.questTab == 2) {
				c.forcedChat("I still have " + c.getSlayer().getTaskAmount() + " more kills to go.");
			}
			if(c.questTab == 3) {
		...
		case 40176:
			if(c.questTab == 0) {
				c.forcedChat("Your inventory value is " + c.getItems().getTotalWorthSTR() + ".");
			//	DailyReward dailyCheck = new DailyReward(c);
			//	dailyCheck.getDay(); //josh check this
			} 
			if(c.questTab == 1) {
				c.getDiaryManager().getMorytaniaDiary().display();
			}
			if(c.questTab == 2) {
				c.forcedChat("I currently have " + c.getBH().getBounties() + " Bounties");
			}
			if(c.questTab == 3) {
		...
		case 40177:
			if(c.questTab == 0) {
				//c.getPA().sendFrame126("https://discord.gg/7WRM23cpZe", 12000);
			} 
			if(c.questTab == 1) {
				c.getDiaryManager().getWesternDiary().display();
			}
			if(c.questTab == 2) {
				c.forcedChat("I currently have " + c.raidPoints + " Raid Points");
			}
			if(c.questTab == 3) {
		...
		case 40178:
			if(c.questTab == 0) {
				c.forcedChat("I have donated a total of $" + c.amDonated + ".");
			} 
			if(c.questTab == 1) {
				c.getDiaryManager().getWildernessDiary().display();
			}
			if(c.questTab == 2) {
				c.forcedChat("I currently have " + c.getAchievements().points + " Achievement Points");
			}
			if(c.questTab == 3) {	
		...
		case 40179: //forums
			if(c.questTab == 0) {
				//c.getPA().sendFrame126("http://YOURLINKHERE.com/forums", 12000);
			} 
			if(c.questTab == 1) {
		...
		case 226162:
			if (c.getRights().isOrInherits(Right.ULTIMATE_IRONMAN)){
				c.sendMessage("@red@You can only deposit untradable items on this game mode!");
		...
		case 75007:
			c.sendMessage("@blu@Use the corresponding enchant spells");
			break;
		...
		case 39241:
			...
			c.forcedChat("I've played for a total of : " + time);
			break;
		case 113240:
			c.forcedChat("I currently have: " + c.pkp + " PK Points.");
			break;
		...
		case 11015:
			if (c.usingLamp) {
				if (c.antiqueLamp && !c.normalLamp) {
					c.usingLamp = false;
					c.getPA().addSkillXP(200000, c.antiqueSelect, true);
			...
			if (c.autocastId > 0) {
				...
				if (c.playerMagicBook == 1) {
					if (c.playerEquipment[c.playerWeapon] == 4675 || c.playerEquipment[c.playerWeapon] == 6914
							|| c.playerEquipment[c.playerWeapon] == 21006|| c.playerEquipment[c.playerWeapon] == 4710
							|| c.playerEquipment[c.playerWeapon] == 12904|| c.playerEquipment[c.playerWeapon] == 22296 || c.playerEquipment[c.playerWeapon] == 22647) {
		...
		case 29063:
			...
			if (checkspec.getRequiredCost() > c.specAmount) {
		...
		case 75010:
			if(c.wildLevel > Config.NO_TELEPORT_WILD_LEVEL) {
			 ...
			 if (Boundary.isIn(c, Boundary.RAIDS) || (Boundary.isIn(c, Boundary.OLM))) {
				...
				//c.getRaids().killAllSpawns();
			 }
			 if (Boundary.isIn(c, Boundary.TOB)) {
					c.getPA().spellTeleport(3219, 4452, 4, true);
					//c.getRaids().killAllSpawns();
				 }
		...
		case 153236: // rigour
			if (c.rigUnlocked == true) {
				...
				c.sendMessage("You have not yet unlocked this prayer!");
		...
		case 153239: // augury
			if (c.augUnlocked == true) {
				...
				c.sendMessage("You have not yet unlocked this prayer!");
				c.getPA().sendFrame36(c.PRAYER_GLOW[28], 0);
			}
			break;
