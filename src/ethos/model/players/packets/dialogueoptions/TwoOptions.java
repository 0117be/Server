...
import ethos.model.content.Lottery;
...
public class TwoOptions {
	...
	public static void handleOption1(Player c) {
		...
		case 8936:
			...
			break;
		case 16740:					
			c.getPA().movePlayer(3121, 3492, 0);
			c.getPA().closeAllWindows();
			c.sendMessage("@red@This is a dangerous zone. You will lose items on death!");
			c.sendMessage("@pur@Good luck!@pur@");
			break;
		...
		case 9030:
			...
			c.getItems().deleteItem(13513, 1);
			c.getPA().closeAllWindows();

			c.bonusXpTime += 3000;
			c.getPA().sendGameTimer(ClientGameTimer.EXPERIENCE, TimeUnit.SECONDS, 1800);
			c.getPA().closeAllWindows();
			}
			break;
		case 648:
		...
		case 9034:
			...
			c.getItems().deleteItem(14945, 1);
			c.getPA().closeAllWindows();
			c.bonusXpTime += 1500;
		
			c.getPA().sendGameTimer(ClientGameTimer.EXPERIENCE, TimeUnit.SECONDS, 900);
			c.getPA().closeAllWindows();
			}
			break;	
			...
			case 9036:
				if (c.getItems().playerHasItem(14947, 1)) {
					c.getItems().deleteItem(14947, 1);
					c.getPA().closeAllWindows();
					c.bonusXpTime += 6000;
					
					c.getPA().sendGameTimer(ClientGameTimer.EXPERIENCE, TimeUnit.SECONDS, 3600);
					c.getPA().closeAllWindows();
					}
					break;
			case 9038:
				if (c.getItems().playerHasItem(14951, 1)) {
				c.getItems().deleteItem(14951, 1);
				c.getPA().closeAllWindows();
				c.bonusXpTime += 9000;
		
				c.getPA().sendGameTimer(ClientGameTimer.EXPERIENCE, TimeUnit.SECONDS, 5400);
				c.getPA().closeAllWindows();
				}
				break;
			case 9040:
				if (c.getItems().playerHasItem(14949, 1)) {
				c.getItems().deleteItem(14949, 1);
				c.getPA().closeAllWindows();
				c.bonusXpTime += 18000;
			
				c.getPA().sendGameTimer(ClientGameTimer.EXPERIENCE, TimeUnit.SECONDS, 10800);
				c.getPA().closeAllWindows();
				}
				break;
				/**
		...
		case 9032:
			...
			c.getItems().deleteItem(13279, 1);
			c.getPA().closeAllWindows();
			c.bonusDropTime += 1500;
			c.getPA().sendGameTimer(ClientGameTimer.DROPS, TimeUnit.SECONDS, 900);
			c.getPA().closeAllWindows();
			}
			break;
		case 9044:
			if (c.getItems().playerHasItem(14955, 1)) {
			c.getItems().deleteItem(14955, 1);
			c.getPA().closeAllWindows();
			c.bonusDropTime += 3000;
			c.getPA().sendGameTimer(ClientGameTimer.DROPS, TimeUnit.SECONDS, 1800);
			c.getPA().closeAllWindows();
			}
			break;
		case 9046:
			if (c.getItems().playerHasItem(14957, 1)) {
			c.getItems().deleteItem(14957, 1);
			c.getPA().closeAllWindows();
			c.bonusDropTime += 6000;
			c.getPA().sendGameTimer(ClientGameTimer.DROPS, TimeUnit.SECONDS, 3600);
			c.getPA().closeAllWindows();
			}
			break;
		case 9048:
			if (c.getItems().playerHasItem(14953, 1)) {
			c.getItems().deleteItem(14953, 1);
			c.getPA().closeAllWindows();
			c.bonusDropTime += 9000;
			c.getPA().sendGameTimer(ClientGameTimer.DROPS, TimeUnit.SECONDS, 5400);
			c.getPA().closeAllWindows();
			}
			break;
		case 9050:
			if (c.getItems().playerHasItem(14959, 1)) {
			c.getItems().deleteItem(14959, 1);
			c.getPA().closeAllWindows();
			c.bonusDropTime += 18000;
			c.getPA().sendGameTimer(ClientGameTimer.DROPS, TimeUnit.SECONDS, 10800);
			c.getPA().closeAllWindows();
			}
			break;
		case 9052:
			...
			break;
		case 5151:
			if (c.getItems().playerHasItem(21034)) {
			...
			break;
		case 5152:
			if (c.getItems().playerHasItem(21079)) {
		...
		case 11824:
	    	...
	    	Achievements.increase(c, AchievementType.HASTA, 1);
			c.nextChat = -1;
			break;
		...
		if (c.dialogueAction == 132) {
			c.getDH().sendDialogues(655, 306);
			c.dialogueAction = -1;
			c.teleAction = -1;
			return;
		}
		if (c.dialogueAction == 133) {
			c.getDH().sendDialogues(656, 306);
			c.dialogueAction = -1;
			c.teleAction = -1;
			return;
		}
		...
		if (c.dialogueAction == 120) {
			...
			loser.getItems().addItem(995, 75000);
			winner.sendMessage(
		...
		if (c.dialogueAction == 4004) {
			...
			if (c.getItems().playerHasItem(2701, 1)) {
				...
				c.gfx100(263);
			    c.amDonated += 75;
				c.getTitles().setCurrentTitle("<col=4d0099><shad=7000>Trusted Dicer</shad>");
				c.getItems().addItemUnderAnyCircumstance(15098, 1);
				c.sendMessage("You are now a Trusted Dicer. A dice has been added to your bank!");
				c.getPA().closeAllWindows();
			}
		}
	...
	public static void handleOption2(Player c) {
		...
		switch (c.dialogueAction) {
		/**
		 * Bonus xp and double drop books
		 */
		case 648:
			c.getPA().closeAllWindows();
			c.nextChat = 649;
