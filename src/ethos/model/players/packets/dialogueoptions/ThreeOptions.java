...
public class ThreeOptions {
	...
	public static void handleOption1(Player c) {
		...
		case 6993:
			if (c.getItems().playerHasItem(2996, 25)) {
		    	...
		    	Achievements.increase(c, AchievementType.SHOPPER, 1);
			}
			break;
		...
		case 6162:
			if (c.bossPoints >= 100) {
				c.bossPoints -=100;
				c.getDH().sendStatement("You have exchanged <col=FF0000>100</col> boss points for <col=0042FF></col>a care package.");
				c.gfx0(164);
				c.getItems().addItem(13187, 1);
		    	//Achievements.increase(c, AchievementType.BLOODMONEY, 1000);
				}
				else
					c.getDH().sendStatement("You don't have enough <col=FF000>boss points</col> to exchange.");
			break;
	...
	public static void handleOption2(Player c) {
		...
		case 6162:
			if (c.pkp >= 100) {
				c.pkp -=100;
				c.getDH().sendStatement("You have exchanged <col=FF0000>100</col> pk points for <col=0042FF></col>a care package.");
				c.getItems().addItem(13187, 1);
				c.gfx0(164);
		    	//Achievements.increase(c, AchievementType.BLOODMONEY, 1000);
				}
				else
					c.getDH().sendStatement("You don't have enough <col=FF000>pk points</col> to exchange.");
			break;
		case 6993:
			c.getPA().removeAllWindows();
			break;
	...
	public static void handleOption3(Player c) {
		...
		case 811: // Deposit
			...
			break;
		case 6162:
			if (c.getBH().bounties >=1_000_000) {
				c.getBH().bounties -= 1_000_000;
				c.getDH().sendStatement("You have exchanged <col=FF0000>1M</col> bounty points for <col=0042FF></col>a care package.");
				c.gfx0(164);
				c.getItems().addItem(13187, 1);
		    	//Achievements.increase(c, AchievementType.BLOODMONEY, 1000);
				}
				else
					c.getDH().sendStatement("You don't have enough <col=FF000>bounty hunter points</col> to exchange.");
			break;
		case 1600:
