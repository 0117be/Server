...
import ethos.model.content.AnguishChest;
import ethos.model.content.BloodAltar;
import ethos.model.content.CrystalChest;
import ethos.model.content.Obelisks;
import ethos.model.content.SlayerChest;
import ethos.model.content.WildernessChest;
...
public class ObjectOptionOne {
	...
	public static void handleOption(final Player c, int objectType, int obX, int obY) {
		...
		if (c.getWerewolfAgility().werewolfCourse(c, objectType)) {
			return;
		}
		if (c.getRoofTopAlkharid().execute(c, objectType)) {
			return;
		}
		if (c.getRoofTopCanifis().execute(c, objectType)) {
			return;
		}
		if (c.getRoofTopFalador1().execute(c, objectType)) {
			return;
		}
		if (c.getRoofTopRelleka().execute(c, objectType)) {
			return;
		}
	...
	//	ObjectDef def = ObjectDef.getObjectDef(objectType);
		...
		if (c.getTob().handleObjectClick(c,objectType)) {
			return;
		}
		...
		switch (objectType) {
	        ...
	        case 7236:
				if (Misc.random(2) == 1) {
					...
					c.getThieving().steal(Stall.Rogue5, objectType, location);
					break;
				}
				break;
			case 22845:
				...
				break;
			case 21181:
				c.sendMessage("@red@Talk to Santa to enter the dungeon, Merry Christmas!");
				break;
			case 1568:
				if (c.absX == 3099) {
					c.getPA().movePlayer(3100, 3879, 0);
				} else if (c.absX == 3100) {
					c.getPA().movePlayer(3099, 3879, 0);
				}
				break;
			case 15617:
			case 19435:
			case 26703:
				...
				c.sendMessage("@whi@You grab a snowball, thow it at someone!");
				break;
		case 31990:
			if (c.absY == 4054) {
			...
			break;
					...
					case 7273:
						c.getPA().movePlayer(3105, 3690, 0);
						break;
			case 31561:
				int reqLvl = 75;
			...
			//message	
				...
				break;
			case 9138:
				c.getPA().movePlayer(2096, 2847, 2);
				break;
			case 31627:
		...
		case 23709:
			...
			c.sendMessage("@blu@You have been healed.");
			c.objectDistance=1;	
			return;
		case 30397:
			if (c.absX ==  3119) {
			c.getDH().sendDialogues(16740, -1);	
			} else if (c.absX ==  3121) {
				c.getPA().movePlayer(3119, 3492, 0);
			}
			return;
		case 29241:			
		...
		case 29772://Tree that gives vials in raids
		  ...
		  case 29495:
			  ...
			  if (c.playerLevel[Skill.SLAYER.getId()] != 99) {
					c.sendMessage("@red@You need 99 Slayer to teleport here.");
					return;
			  }
			  if (!c.getItems().playerHasItem(3339, 10)) {
					c.sendMessage("@red@You need 10 Blood Shards to teleport.");
					return;
			  }

			  else {
					c.getItems().deleteItem(3339, 10);
					c.getPA().movePlayer(1171, 9943, 0);
					Server.npcHandler.spawnNpc(c, 6298, 1172, 9937, 0, 1, 500, 5, 250, 250, true, false);
					c.sendMessage("@red@Akeldama has spawned, kill her and retrieve the blood vial!");
			  }
					break;
		  case 32507:
			  ...
			  if (c.playerLevel[Skill.SLAYER.getId()] != 99) {
					...
					c.getPA().movePlayer(1171, 9943);
					Server.npcHandler.spawnNpc(c, 6298, 1172, 9937, 0, 1, 55, 5, 250, 250, true, false);
					c.sendMessage("@red@Akeldama has spawned, kill her and retrieve the blood vial!");
					break;
		 ...
		 */
			...
			case 26812:
			case 32546:
				c.getDH().sendDialogues(1600, -1);
				c.sendMessage("@red@ ::vote every 12 hours for rewards!");
				break;
				case 31556: //northern Rev Cave entrance
					if (c.absX ==  3126) {
					c.getPA().movePlayer(3241, 10234);
					}
					break;
				case 31555: //southern Rev Cave entrance
					if (c.absY ==  3653) {
					c.getPA().movePlayer(3196, 10056);
					}
					break;
				case 31557:
					c.getPA().movePlayer(3074, 3653);
					break;
				case 31558:
		...
		case 9728:
			if (c.absX == 3111) {
				c.getPA().movePlayer(3079, 9808, 0);
			} else {
		...
		case 25014:
			if (Boundary.isIn(c, Boundary.PURO_PURO)) {
				c.getPA().startTeleport(3098, 3506, 0, "puropuro", false);
			} else {
		...
		case 2147:
			if (obX >= 3093 && obY <= 3506) { // Edge Dungeon Ladder
		...
		case 10805:// drake stairs
			c.getPA().movePlayer(1334, 10205, 1);
			break;
		case 10806:// drake stairs
		...
		/*	if((obX >= 3020 && obY <= 9739) || (obX >= 3019 && obY <= 9738) || (obX >= 3018 && obY <= 9739) || (obX >= 3019 && obY <= 9742)) { // Mining Guild
			...
			break;
		case 17384:
			if (obX >= 3097 && obY <= 9867) { // Edge Dungeon Ladder to Hill Giants
				AgilityHandler.delayEmote(c, "CLIMB_DOWN", 3117, 9852, 0, 2);
				}
			break;
		case 30258:
		/*	if((obX >= 3020 && obY <= 9739) || (obX >= 3019 && obY <= 9738) || (obX >= 3018 && obY <= 9739) || (obX >= 3019 && obY <= 9742)) { // Mining Guild
				AgilityHandler.delayEmote(c, "CLIMB_UP", 3021, 3338, 0, 2);}*/
			if (obX == 1452 && obY == 10068) { // Edge Dungeon Ladder
				AgilityHandler.delayEmote(c, "CLIMB_UP", 1451, 10069, 2, 2);}
			if (obX == 1437 && obY == 10094) {		
				AgilityHandler.delayEmote(c, "CLIMB_UP", 1438, 10093, 2, 2);}
			if (obX == 1458 && obY == 10095) {		
				AgilityHandler.delayEmote(c, "CLIMB_UP", 1457, 10095, 1, 2);}
			break;
		case 30259:
			if (obX == 1452 && obY == 10068) {		
				AgilityHandler.delayEmote(c, "CLIMB_UP", 1451, 10069, 3, 2);}
			if (obX == 1437 && obY == 10094) {		
				AgilityHandler.delayEmote(c, "CLIMB_UP", 1437, 10093, 3, 2);}
			if (obX == 1458 && obY == 10095) {		
				AgilityHandler.delayEmote(c, "CLIMB_UP", 1457, 10095, 2, 2);}
			break;
		case 30367:
	...
	//		if (c != null) {
		...
		case 3840: // Compost Bin
			...
			break;
		case 7836: // Compost Bin
			c.getFarming().handleCompostAdd();
			break;
		case 7837: // Compost Bin
			c.getFarming().handleCompostAdd();
			break;
		case 7808: // Compost Bin
			c.getFarming().handleCompostAdd();
			break;
		...
		case 733:
			...
			if (Misc.random(chance) == 0) {
				...
				} else if (c.objectX == 3115 && c.objectY == 3859 || c.objectX == 3115 && c.objectY == 3859) {
					objectOne = new GlobalObject(734, obX, obY, c.heightLevel, 3, 10, 50, 733);
				} else if (c.objectX == 3030 && c.objectY == 3852 || c.objectX == 3030 && c.objectY == 3852) {
					objectOne = new GlobalObject(734, obX, obY, c.heightLevel, 3, 10, 50, 733);
				}
