...
public class ItemAssistant {
		...
		for (int i = 0; i < c.playerItems.length; i++) {
			...
			if (killer != null && killer instanceof Player) {
				...
				if (isTradable(c.playerItems[i] - 1) && c.playerItems[i] - 1 != 13307) {
					if (playerKiller.getMode().isItemScavengingPermitted() && !playerKiller.getRights().isOrInherits(Right.IRONMAN) ||  !playerKiller.getRights().isOrInherits(Right.BRNZ) || !playerKiller.getRights().isOrInherits(Right.ULTIMATE_IRONMAN) || !playerKiller.getRights().isOrInherits(Right.HARDCORE) || playerKiller.inClanWars()) {
		...
		for (int e = 0; e < c.playerEquipment.length; e++) {
			...
			if (killer != null && killer instanceof Player) {
				...
				if (isTradable(c.playerEquipment[e])) {
					if (playerKiller.getMode().isItemScavengingPermitted() && !playerKiller.getRights().isOrInherits(Right.IRONMAN)  || !playerKiller.getRights().isOrInherits(Right.BRNZ) ||!playerKiller.getRights().isOrInherits(Right.ULTIMATE_IRONMAN) || !playerKiller.getRights().isOrInherits(Right.HARDCORE)) {
	...
	public boolean isTradable(int itemId) {
		if (itemId == 12899 && c.getToxicTridentCharge() > 0 || itemId == 22323 && c.getSangStaffCharge() > 0 || itemId == 11907 && c.getTridentCharge() > 0) {
			c.sendMessage("You cannot trade this item whilst it has a charge.");
			return false;
		}
	...
	public void sendWeapon(int Weapon, String WeaponName) {
		...
		if (WeaponName.equals("Unarmed")) {
			...
			c.getPA().sendFrame126(WeaponName, 12293);
				...
				|| WeaponName.startsWith("seercull") || WeaponName.contains("blowpipe") || WeaponName.contains("ballista") || WeaponName.contains("thrownaxe") || WeaponName.equals("Dragon knife") || WeaponName.contains("chinchompa")) {
		...
		} else if (WeaponName2.startsWith("dagger") || WeaponName2.contains("anchor") || WeaponName2.contains("sword") || WeaponName2.contains("byssal dagger")  || WeaponName2.contains("rapier")) {
			...
			c.getPA().sendFrame246(8461, 200, Weapon);
			c.getPA().sendFrame126(WeaponName, 8463);
		} else if (WeaponName2.startsWith("spear") || c.playerEquipment[c.playerWeapon] == 22610 || 
				WeaponName2.toLowerCase().contains("zamorakian") || WeaponName2.toLowerCase().contains("hasta")) {
		...
		} else if (WeaponName2.contains("warhammer") || c.playerEquipment[c.playerWeapon] == 4153 || c.playerEquipment[c.playerWeapon] == 7807 || c.playerEquipment[c.playerWeapon] == 13263 || c.playerEquipment[c.playerWeapon] == 12848 || c.playerEquipment[c.playerWeapon] == 21003 || c.playerEquipment[c.playerWeapon] == 22622) {
	...
	public boolean is2handed(String itemName, int itemId) {
		...
		if (itemName.contains("godsword") || itemName.contains("crystal") || itemName.contains("aradomin sword")
				...
				itemName.contains("elder maul") || itemName.contains("bulwark") || itemName.contains("claws") ||
				itemName.contains("vitur")) {
			return true;
		}
		switch (itemId) {
		case 12926:
	...
	public boolean wearItem(int wearID, int slot) {
		...
		if (c.playerItems[slot] == (wearID + 1)) {
			...
			case 7927:
				...
				c.npcId2 = wearID == 7927 ? 5542 : 2188;
				c.isNpc = true;
				c.updateRequired = true;
				c.morphed = true;
				c.setAppearanceUpdateRequired(true);
			...
			if (targetSlot == -1 || !item.isWearable()) {
				if (wearID >= 5509 && wearID <= 5512 || wearID == 6819 || wearID == 21347 || wearID == 15098 || wearID == 11918 || wearID == 13656 || wearID == 7959 || wearID == 7960) {
					return false;
